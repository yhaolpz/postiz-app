...
const fps = 30;
const tailPad = 0.3;
const outputName = 'ai-agent-context-production-v4.zh-CN.mp4';
...
function balancedLines(value, maxChars = 20) {
  ...
  const chars = [...text];
  const measuredLength = chars.filter((char) => !/\s/.test(char)).length;
  if (measuredLength <= maxChars) return [text];
  const candidates = [];
  chars.forEach((char, index) => {
    if ('·，、：；'.includes(char) && index >= 3 && chars.length - index - 1 >= 3) candidates.push(index + 1);
  });
  for (const keyword of ['信息', '工具', '细节', '笔记', '规则', '目标', '示例', '状态', '证据', '上下文']) {
    const keywordIndex = text.indexOf(keyword);
    const splitIndex = keywordIndex < 0 ? -1 : [...text.slice(0, keywordIndex + keyword.length)].length;
    if (splitIndex >= 3 && chars.length - splitIndex >= 3) candidates.push(splitIndex);
  }
  const splitAt = candidates.length
    ? candidates.sort((a, b) => Math.abs(a - chars.length / 2) - Math.abs(b - chars.length / 2))[0]
    : Math.max(4, Math.min(chars.length - 4, Math.round(chars.length / 2)));
  return [chars.slice(0, splitAt).join('').trim(), chars.slice(splitAt).join('').trim()].filter(Boolean);
}
...
function featuredObject(scene, className = '') {
  ...
  return `<div class="featured-object visual-object ${className}"><img src="${propSrc(scene.props[0].id)}" alt=""><div class="object-label">${labelMarkup(scene.coreLabel, 20)}</div></div>`;
}
...
function renderScene(scene, scenePlan) {
  ...
  if (scene.layout === 'agent-center' || scene.layout === 'human-center') {
    ...
    return `${open}<h1 class="scene-headline ${headlineClass}">${esc(scene.headline)}</h1><div class="solo-stage">${actorMarkup(scene, kind, 'motion-target')}<div class="solo-callout">${labelMarkup(scene.coreLabel, 20)}</div></div></section>`;
  }
  if (scene.layout === 'two-props') return `${open}<h1 class="scene-headline ${headlineClass}">${esc(scene.headline)}</h1><div class="pair-stage">${propMarkup(scene.props[0])}<div class="relation-mark">≠</div>${propMarkup(scene.props[1], 'motion-target')}</div><div class="pair-label">${labelMarkup(scene.coreLabel, 24)}</div></section>`;
...
.generated-stage{height:700px;position:relative;display:grid;place-items:center}.generated-art{width:720px;height:720px;object-fit:contain;border:0;border-radius:0;box-shadow:none}.generated-label{position:absolute;right:70px;bottom:84px;width:620px;padding:20px 26px;border:7px solid var(--ink);border-radius:24px;background:var(--yellow)}
