...
function motionTargetsForScene(scene) {
  ...
  const accent = { role: 'marker', selector: `${root} .scene-motion-accent`, targetClass: 'accent', maximumActions: 2 };
  ...
  const primary = { role: 'core-object', selector: `${root} .motion-target`, targetClass: scene.layout === 'generated' ? 'large' : 'prop', maximumActions: scene.layout === 'generated' ? 1 : 2 };
...
function buildSemanticMotionPlan(cues, scenePlan) {
  ...
  for (const scene of eligibleScenes) {
    ...
    const targets = motionTargetsForScene(scene);
    ...
    selected.forEach((cue, nodeIndex) => {
      ...
      const targetOptions = [...targets.slice((nodeIndex + scene.paragraphNumber) % targets.length), ...targets.slice(0, (nodeIndex + scene.paragraphNumber) % targets.length)];
      let selectedPair = null;
      for (const actionSpec of actionOptions) {
        const preferredTargets = actionSpec.action === 'claim-underline' || actionSpec.action === 'source-path-draw'
          ? [...targetOptions].sort((a, b) => Number(b.role === 'marker') - Number(a.role === 'marker'))
          : targetOptions;
        const target = preferredTargets.find((candidate) => {
  ...
  const requiredFields = ['id', 'sceneId', 'at', 'triggerCueId', 'triggerText', 'target', 'targetSelector', 'action', 'actionFamily', 'motionSignature', 'direction', 'fromState', 'peakState', 'toState', 'magnitude', 'easing', 'entranceType', 'semanticPurpose', 'duration', 'holdUntil'];
...
function renderScene(scene, scenePlan) {
  ...
  if (scene.layout === 'agent-center' || scene.layout === 'human-center') {
    ...
    return `${open}<h1 class="scene-headline ${headlineClass}">${esc(scene.headline)}</h1><div class="solo-stage">${actorMarkup(scene, kind, 'motion-target')}<div class="solo-callout yellow-highlight">${labelMarkup(scene.coreLabel, profile.visual.yellowHighlightMaxMeasuredCharsPerLine)}</div></div>${close}`;
  }
