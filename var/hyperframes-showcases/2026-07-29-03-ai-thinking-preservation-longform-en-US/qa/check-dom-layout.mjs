...
const result = await page.evaluate(({ openingReadability, locale }) => {
  ...
  if (activeAccentRun) accentRuns.push(activeAccentRun);
  const expectedAccents = uniformTypography.accentTokens?.[locale] ?? [];
  const typography = {
  ...
  openingReadability,
  locale: JSON.parse(readFileSync(path.join(projectDir, 'episode.json'), 'utf8')).locale
});
