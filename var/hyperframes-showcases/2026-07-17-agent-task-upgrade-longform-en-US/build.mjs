...
async function mergeAudio(chapters) {
  ...
  execFileSync('ffmpeg', [
    '-y', ...inputs,
    '-filter_complex', `${filterInputs.join('')}concat=n=${chapters.length}:v=0:a=1,loudnorm=I=-17:LRA=7:TP=-1.5[a]`,
...
function assignVisualVariants(scenes) {
  ...
  return scenes.map((scene) => {
    ...
    return {
      ...scene,
      visualVariant: mirrorableLayouts.has(scene.layout) && layoutCount % 2 === 1
        ? 'mirror'
        : 'standard',
    };
  });
...
function renderPhaseLegacy(scene, index) {
  ...
  return `
      <section data-hf-id="hf-phase-${scene.id}" id="phase-${scene.id}" class="phase${index === 0 ? ' is-first' : ''}" data-chapter="${escapeHtml(scene.chapter)}">
        ...
        <img data-hf-id="hf-${scene.id}-human" class="actor human-actor" src="assets/pack/sprites/human/${escapeHtml(scene.human)}.png" alt="Engineer ${escapeHtml(scene.human)}">
...
function sceneLayoutGeometry(scene) {
  ...
  if (scene.visualVariant !== 'mirror' || !['hero', 'focus', 'grid', 'stage'].includes(scene.layout)) {
    return geometry;
  }
...
function renderPhaseV2(scene, index) {
  if (scene.recap) {
    ...
    const summaryTitle = scene.finalSummary ? 'Delegate work\nKeep control' : scene.chapter;
    const summaryCta = scene.finalSummary ? `
          <div class="summary-cta"><strong>Tiny Agent</strong><span>Practical methods for reliable AI and agent work</span></div>` : '';
    return `
      ...
      <section data-hf-id="hf-v2-phase-${scene.id}" id="v2-phase-${scene.id}" class="phase layout-${escapeHtml(scene.layout)} variant-${escapeHtml(scene.visualVariant || 'standard')}${scene.showIntro ? ' has-chapter-intro' : ''}${index === 0 ? ' is-first' : ''}" style="z-index:${index + 1}" data-chapter="${escapeHtml(scene.chapter)}" data-layout-allow-overlap data-layout-allow-overflow data-layout-allow-occlusion>
        <div class="scene-content-shell">
          <div class="balance-layer" style="transform:translate(${scene.balanceShift?.x || 0}px,${scene.balanceShift?.y || 0}px)">
            <div class="ground-line"></div>
            ${comparison}
            <img data-hf-id="hf-v2-${scene.id}-human" class="actor human-actor" style="${frameStyle(geometry.human)}" src="assets/pack/sprites/human/${escapeHtml(scene.human)}.png" alt="Engineer ${escapeHtml(scene.human)}">
    ...
    <template id="story-template">
      <div data-hf-id="hf-v2-story-root" id="v2-story-root" data-composition-id="story" data-width="1920" data-height="1080" data-start="0" data-duration="${duration.toFixed(3)}">
        <style>
          ...
          #v2-story-root .phase { position:absolute; inset:0; width:1920px; height:1080px; overflow:hidden; background-color:#fcfdfc; background-image:linear-gradient(rgba(17,20,19,.035) 1px,rgba(252,253,252,0) 1px),linear-gradient(90deg,rgba(17,20,19,.035) 1px,rgba(252,253,252,0) 1px); background-size:48px 48px; opacity:0; transform-origin:center center; }
          ...
          #v2-story-root .compare-panel { position:absolute; top:228px; z-index:6; width:690px; height:540px; border:4px solid #dfe7e4; background:rgba(252,253,252,.94); }
          ...
          #v2-story-root .chapter-intro { position:absolute; inset:0; z-index:80; overflow:hidden; background:#fcfdfc; clip-path:inset(0 0 0 0); }
          ...
          #v2-story-root .chapter-summary { position:absolute; inset:0; z-index:90; display:grid; grid-template-columns:440px 1fr; background:#fcfdfc; clip-path:inset(0 0 0 100%); }
...
function renderIndex({ duration, chapters, captions, scenes }) {
  const captionMarkup = captions.map((caption, index) => `
        <div data-hf-id="hf-caption-${index + 1}" id="caption-${index + 1}" class="narration-caption${index === 0 ? ' is-first' : ''}">${escapeHtml(caption.text)}</div>`).join('');
...
<html lang="en-US">
  <head>
    ...
    <style>
      ...
      * { box-sizing:border-box; }
      ...
      .narration-caption { position:absolute; inset:0; display:flex; align-items:center; justify-content:center; padding:10px 54px 4px; font-size:35px; font-weight:700; line-height:1.38; text-align:center; opacity:0; }
      .narration-caption.is-first { opacity:1; }
      ...
      .progress-segment { flex:var(--chapter-weight) 1 0; min-width:120px; }
  ...
  <body>
    <div data-hf-id="hf-main-root" id="root" data-composition-id="main" data-start="0" data-duration="${duration.toFixed(3)}" data-width="1920" data-height="1080">
      <div data-hf-id="hf-board" class="board-fill"></div>
      <div data-hf-id="hf-story-slot" id="story-slot" class="story-slot" data-composition-id="story" data-composition-src="compositions/story.html" data-start="0" data-duration="${duration.toFixed(3)}" data-track-index="1" data-layout-allow-overlap data-layout-allow-overflow></div>
      <header data-hf-id="hf-masthead" class="masthead"><div class="brand">Tiny Agent</div><h1 class="title">Which Tasks Deserve an AI Agent?</h1></header>
      <div data-hf-id="hf-caption-band" class="caption-band">${captionMarkup}
      </div>
      <nav data-hf-id="hf-chapter-progress" class="chapter-progress" aria-label="Video chapter progress">${progressMarkup}
      </nav>
      <audio data-hf-id="hf-narration" id="narration" class="clip" data-start="0" data-duration="${duration.toFixed(3)}" data-track-index="10" data-media-start="0" data-volume="1" src="assets/audio/narration.en.normalized.mp3"></audio>
    </div>
    <script>
      window.__timelines = window.__timelines || {};
      const tl = gsap.timeline({ paused: true });
...
async function compile() {
  ...
  await writeFile(path.join(projectRoot, 'index.html'), renderIndex({ duration, chapters: chapterTimings, captions, scenes }));
  ...
  await writeFile(path.join(projectRoot, 'summary.json'), `${JSON.stringify({
    title: 'Which Tasks Deserve an AI Agent?',
    source: sourcePath,
    language: 'en-US',
    speech: { provider: 'edge-tts', voice, rate, integratedLoudnessTarget: '-17 LUFS' },
    ...
    output: { file: 'renders/video.mp4', status: 'pending' },
  }, null, 2)}\n`);
