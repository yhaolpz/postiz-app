...
for (const scene of timingMap.scenes.filter((item) => item.transition)) {
  ...
  for (const actor of ['agent']) {
    ...
    const timelineSetLine = findLine(`tl.set("${selector}"`);
    const immediateSetLine = findLine(`gsap.set("${selector}"`);
    const setLine = timelineSetLine || immediateSetLine;
    ...
    const setAt = immediateSetLine ? 0 : timelineTime(setLine);
    const moveAt = timelineTime(toLine);
    const expectedMoveAt = scene.transition.agentAt;
    ...
    const pass = setAt <= scene.transition.contentRevealAt + 0.001
      ...
      && (!immediateSetLine || scene.transition.contentRevealAt === 0);
...
const report = {
  policy: 'prime-before-reveal-retention-v2-single-agent',
