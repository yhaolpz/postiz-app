...
function applyRetentionScenePlan(plan, animationPlan) {
  ...
  plan.chapters[0].scenes[0].callout = 'AI 智能体为什么忘记长期任务';
  plan.chapters[0].scenes[1].callout = '上下文是一份有限的注意力预算';
  plan.chapters[0].scenes[2].callout = 'AI 智能体六部分高信号包';
  animationPlan.scenes['c01-s01'].beats[0].trigger = '你的 AI 智能体为什么';
  animationPlan.scenes['c01-s02'].beats[0].trigger = '所以，喂得更多';
  animationPlan.scenes['c01-s03'].beats[0].trigger = '核心判断只有一句';
}
...
async function generateTts() {
  ...
  await rm(path.join(audioDir, 'narration.zh.normalized.mp3'), { force: true });
...
function renderPhaseV2(scene, index) {
  if (scene.recap) {
    ...
    const summaryTitle = scene.finalSummary ? 'AI 智能体上下文\n保留信号，减少噪声' : scene.chapter;
    const summaryCta = '';
    return `
...
<html lang="zh-CN">
  ...
  <body>
    <div data-hf-id="hf-main-root" id="root" data-composition-id="main" data-start="0" data-duration="${duration.toFixed(3)}" data-width="1920" data-height="1080">
      <img data-hf-id="hf-opening-cover" id="opening-cover" class="opening-cover" src="thumbnails/thumbnail.zh-CN.png" alt="AI 智能体为什么会忘记长期任务？" data-layout-allow-occlusion>
      ...
      <header data-hf-id="hf-masthead" class="masthead content-layer"><div class="brand">Tiny Agent</div><h1 class="title">AI 智能体为什么会忘：用高信号上下文完成长期任务</h1></header>
...
async function compile() {
  ...
  await writeFile(path.join(projectRoot, 'summary.json'), `${JSON.stringify({
    title: 'AI 智能体为什么会忘：用高信号上下文完成长期任务',
    source: sourcePath,
    language: 'zh-CN',
    ...
    output: { file: 'renders/2026-07-21-03-agent-context-engineering-retention-v2.zh-CN.mp4', status: 'pending' },
  }, null, 2)}\n`);
