...
function buildHookTiming(firstSegment) {
  ...
  if (episode.locale.startsWith('zh')) {
    ...
    const glyphs = chars.map((char, index) => {
      const at = start + totalDuration * (cursorWeight / weightTotal);
      cursorWeight += /[，。！？!?]/.test(char) ? 0.55 : 1;
      return { id: `hook-glyph-${index + 1}`, text: char, at: Number(at.toFixed(3)), wordIndex: index, letterIndex: 0 };
    });
    return { mode: 'character', source: 'final edge-tts VTT audible range', earlyRevealCount: 0, glyphs };
  }
...
function validateRules(chapters, segments, scenePlan) {
  ...
  for (const chapter of scenePlan.chapters.slice(1, -1)) {
    ...
    if (chapter.scenes.at(-1)?.type !== 'recap' || chapter.scenes.filter((scene) => scene.type === 'recap').length !== 1) throw new Error(`${chapter.label} must end with one synthesis recap.`);
    if (!chapter.scenes.at(-1)?.narration.startsWith(profile.audio.chapterRecapSpokenPrefix)) throw new Error(`${chapter.label} recap must start with ${profile.audio.chapterRecapSpokenPrefix}.`);
    if (chapter.scenes.at(-1)?.recapPoints?.length !== 3) throw new Error(`${chapter.label} recap must expose three visible anchors.`);
  }
...
function generateTts() {
  ...
  writeFileSync(path.join(projectDir, 'qa/recap-report.json'), `${JSON.stringify({
    pass: scenePlan.chapters.slice(1, -1).every((chapter) => chapter.scenes.at(-1)?.type === 'recap' && chapter.scenes.at(-1)?.recapPoints?.length === 3),
    chapters: scenePlan.chapters.slice(1, -1).map((chapter) => ({ label: chapter.label, sceneId: chapter.scenes.at(-1)?.id, points: chapter.scenes.at(-1)?.recapPoints }))
  }, null, 2)}\n`);
  writeFileSync(path.join(projectDir, 'qa/recap-spoken-prefix-report.json'), `${JSON.stringify({
    pass: scenePlan.chapters.slice(1, -1).every((chapter) => chapter.scenes.at(-1)?.narration.startsWith(profile.audio.chapterRecapSpokenPrefix)),
    requiredPrefix: profile.audio.chapterRecapSpokenPrefix,
    chapters: scenePlan.chapters.slice(1, -1).map((chapter) => ({ label: chapter.label, sceneId: chapter.scenes.at(-1)?.id, narration: chapter.scenes.at(-1)?.narration }))
  }, null, 2)}\n`);
  ...
  writeFileSync(path.join(projectDir, 'qa/cognitive-load-report.json'), `${JSON.stringify({
    ...
    recapMode: profile.cognitiveLoad.requiredRecapMode
  }, null, 2)}\n`);
