...
const checks = {
  transitions() {
    ...
    if (timing.checkpoints.hookHardCut - timing.checkpoints.firstSentenceEnd > profile.cognitiveLoad.knowledgeUnitPauseSeconds + 1 / 30) failures.push('kinetic hook pause exceeds the approved knowledge-unit pause');
  ...
  semantics() {
    ...
    plan.chapters.slice(1, -1).forEach((chapter) => {
      ...
      if (chapter.scenes.at(-1)?.type !== 'recap' || chapter.scenes.filter((scene) => scene.type === 'recap').length !== 1) failures.push(`${chapter.label} lacks one synthesis recap`);
      if (chapter.scenes.at(-1)?.recapPoints?.length !== 3) failures.push(`${chapter.label} recap lacks three visible anchors`);
      if (!chapter.scenes.at(-1)?.narration.startsWith(profile.audio.chapterRecapSpokenPrefix)) failures.push(`${chapter.label} recap lacks the fixed spoken prefix`);
    });
  ...
  balance() {
    const tokens = [
      ...
      '.generated-art{width:720px;height:680px;object-fit:contain;border:0;border-radius:0;box-shadow:none}',
