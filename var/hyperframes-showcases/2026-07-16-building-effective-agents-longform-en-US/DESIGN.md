# Tiny Agent full-length English explainer

## Format

- Canvas: 1920 x 1080, 16:9, 30 fps.
- Target: a complete English YouTube explainer based on `Building effective agents`.
- Voice: `en-US-AnaNeural` at the fixed production rate `+30%`.
...
6. The edge-to-edge chapter rail uses actual narration timings and shows numbered `Preface`, content, and `Summary` sections.
7. The final CTA remains on the full-video summary page; no body scene returns after the summary begins.
...
- Near-white board, faint 48 px grid, near-black marker lines, blue structure accents, yellow decisions, and red warnings.
