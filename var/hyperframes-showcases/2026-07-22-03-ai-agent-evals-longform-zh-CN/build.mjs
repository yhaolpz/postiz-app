...
function layoutPropPositions(scene) {
  ...
  if (Number.isFinite(scene.propShiftX) && scene.propShiftX !== 0) {
    positions = positions.map((position) => ({ ...position, x: position.x + scene.propShiftX }));
  }
...
function renderPhaseV2(scene, index) {
  if (scene.recap) {
    ...
    const summaryTitle = scene.finalSummary ? 'AI 智能体验收\n验证结果，不只听完成' : scene.chapter;
    const summaryCta = '';
    return `
...
<html lang="zh-CN">
  ...
  <body>
    <div data-hf-id="hf-main-root" id="root" data-composition-id="main" data-start="0" data-duration="${duration.toFixed(3)}" data-width="1920" data-height="1080">
      ...
      <header data-hf-id="hf-masthead" class="masthead content-layer"><div class="brand">Tiny Agent</div><h1 class="title">AI 智能体验收：验证结果，不只听完成</h1></header>
...
async function compile() {
  ...
  await writeFile(path.join(projectRoot, 'summary.json'), `${JSON.stringify({
    ...
    output: { file: 'renders/2026-07-22-03-ai-agent-evals-longform.zh-CN.mp4', status: 'pending' },
  }, null, 2)}\n`);
