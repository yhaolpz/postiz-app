...
长期任务并不会因为智能体循环运行就天然连续。每个上下文窗口都可能像一次新的交接班。每次重启都要先定位现在的真实状态。
...
有了底座后，把下一步缩小。AI Agent 做长期任务时，每轮应完成一个有意义的小单元，不要试图一口吞完整项目。这样才能在有限窗口里保持判断质量。
...
把决定和证据写在一起。只写选 B 还不够；要说明 B 通过了这个浏览器检查，而 A 在此处失败，这样才可复用。证据越靠近决定，交接越可靠。
...
当任务跨会话、跨人或跨上下文窗口时，就使用 AI Agent 长期任务进度与交接模板。这样复杂工作也能保持可见、可接手。
