...
const hookKineticEntrance = openingReadability.kineticGlyphEntrance;
if (!hookKineticEntrance?.regularGlyph?.entryVectorsPx?.length || !hookKineticEntrance?.finalQuestionGlyph) {
  throw new Error('Active Tiny Agent profile is missing kineticGlyphEntrance.');
}
const hookQuestionQuality = openingReadability.hookQuestionQuality;
...
function buildSegments(chapters) {
  return chapters.flatMap((chapter) => chapter.paragraphs.flatMap((paragraph, paragraphIndex) => (chapter.index === 0 && paragraphIndex === 0 ? [paragraph] : splitUtterances(paragraph)).map((text, utteranceIndex) => ({
...
function evaluateOpeningHookQuality() {
  const visibleQuestion = String(episode.hookQuestion ?? episode.firstSentence ?? '').trim();
  const review = episode.openingHookReview ?? {};
  const locale = episode.locale ?? '';
  const isChinese = locale.startsWith('zh');
  const allowedIntent = hookQuestionQuality.allowedIntents.includes(review.intent);
  const causalOrDiscoveryForm = isChinese
    ? /(为什么|怎么|如何|何时|什么时候|哪一步|哪些.*(?:导致|造成|让|使)|什么.*(?:导致|造成|让|使))/.test(visibleQuestion)
    : /(^|\b)(why|how|when|where|which|what)\b/i.test(visibleQuestion);
  const obviousYesNoForm = isChinese
    ? /(?:会|能|是否|是不是|有没有).{0,24}吗[？?]$/.test(visibleQuestion)
    : /^(?:do|does|did|is|are|can|will|should|would|has|have)\b/i.test(visibleQuestion);
  const topicIdentityPresent = /\bAI\s+Agents?\b|智能体/i.test(visibleQuestion);
  const questionMatchesReview = review.visibleQuestion === visibleQuestion;
  const audiencePainPointPresent = typeof review.audiencePainPoint === 'string' && review.audiencePainPoint.trim().length >= 12;
  const knowledgeGapPresent = typeof review.knowledgeGap === 'string' && review.knowledgeGap.trim().length >= 12;
  const rejectedObviousQuestionPresent = typeof review.rejectedObviousQuestion === 'string'
    && review.rejectedObviousQuestion.trim().length > 0
    && review.rejectedObviousQuestion.trim() !== visibleQuestion;
  const unresolvedCuriosity = review.obviousAnswerRisk === 'none'
    && typeof review.curiosityRationale === 'string'
    && review.curiosityRationale.trim().length >= 12;
  const normalizeSemanticText = (value) => String(value ?? '').replace(/\s+/g, '').toLocaleLowerCase();
  const directTopicTerms = Array.isArray(review.directTopicTerms)
    ? review.directTopicTerms.map((term) => String(term).trim()).filter(Boolean)
    : [];
  ...
  const directEpisodeTopic = concreteTopicTerms.length > 0
    && concreteTopicTerms.every((term) => normalizeSemanticText(visibleQuestion).includes(normalizeSemanticText(term)));
  const viewerValueOrCuriosity = typeof review.viewerValue === 'string'
    && review.viewerValue.trim().length >= 12
    && (knowledgeGapPresent || unresolvedCuriosity);
  const noTangentialScenarioSetup = review.tangentialSetupRisk === 'none'
    && typeof review.topicAlignmentRationale === 'string'
    && review.topicAlignmentRationale.trim().length >= 12;
  const pass = questionMatchesReview
    && allowedIntent
    && causalOrDiscoveryForm
    && !obviousYesNoForm
    && (!hookQuestionQuality.requireTopicIdentity || topicIdentityPresent)
    && (!hookQuestionQuality.requireAudiencePainPoint || audiencePainPointPresent)
    && (!hookQuestionQuality.requireUnresolvedCuriosity || unresolvedCuriosity)
    && (!hookQuestionQuality.requireDirectEpisodeTopic || directEpisodeTopic)
    && (!hookQuestionQuality.requireViewerValueOrCuriosity || viewerValueOrCuriosity)
    && (!hookQuestionQuality.forbidTangentialScenarioSetup || noTangentialScenarioSetup)
    && knowledgeGapPresent
    && rejectedObviousQuestionPresent
    && /[？?]$/.test(visibleQuestion);
  return {
    pass,
    visibleQuestion,
    intent: review.intent ?? null,
    audiencePainPoint: review.audiencePainPoint ?? null,
    knowledgeGap: review.knowledgeGap ?? null,
    curiosityRationale: review.curiosityRationale ?? null,
    directTopicTerms,
    viewerValue: review.viewerValue ?? null,
    topicAlignmentRationale: review.topicAlignmentRationale ?? null,
    tangentialSetupRisk: review.tangentialSetupRisk ?? null,
    obviousAnswerRisk: review.obviousAnswerRisk ?? null,
    rejectedObviousQuestion: review.rejectedObviousQuestion ?? null,
    checks: {
      questionMatchesReview,
      allowedIntent,
      causalOrDiscoveryForm,
      noObviousYesNoForm: !obviousYesNoForm,
      topicIdentityPresent,
      audiencePainPointPresent,
      knowledgeGapPresent,
      rejectedObviousQuestionPresent,
      unresolvedCuriosity,
      directEpisodeTopic,
      viewerValueOrCuriosity,
      noTangentialScenarioSetup,
      terminalQuestionMark: /[？?]$/.test(visibleQuestion)
    }
  };
}

function assertOpeningHookQuality() {
  const report = evaluateOpeningHookQuality();
  if (!report.pass) throw new Error(`Opening hook quality failed: ${JSON.stringify(report.checks)}`);
  return report;
}
...
function buildHookTiming(firstSegment) {
  ...
  const buildChineseGlyphs = () => {
    ...
    const glyphs = chars.map((char, index) => {
      ...
      const isFinalGlyph = index === chars.length - 1;
      const at = audibleAt - visualLeadSeconds - (isFinalGlyph ? 0.02 : 0);
  ...
  const buildEnglishGlyphs = () => {
    ...
    const words = tokens.map((token, wordIndex) => {
      const measuredWordAnchor = wordIndex === tokens.length - 1 ? wordIndex + 0.25 : wordIndex;
      const audibleAt = literalQuestionAudibleStart + audibleSpan * (measuredWordAnchor / tokens.length);
...
function generatedArtAlphaEvidence(generatedScenes) {
  ...
  for (const scene of generatedScenes) {
    const sceneIds = byAsset.get(scene.generatedArt) ?? [];
    sceneIds.push(scene.id);
    byAsset.set(scene.generatedArt, sceneIds);
  }
  return [...byAsset.entries()].map(([asset, sceneIds]) => {
    const file = path.join(projectDir, 'assets/generated/scene-art', asset);
    const [width, height] = run('magick', [file, '-format', '%w %h', 'info:']).split(/\s+/).map(Number);
    const alpha = run('magick', [file, '-alpha', 'extract', '-format', `%[fx:round(p{0,0}.r*255)] %[fx:round(p{${width - 1},0}.r*255)] %[fx:round(p{0,${height - 1}}.r*255)] %[fx:round(p{${width - 1},${height - 1}}.r*255)]`, 'info:']).split(/\s+/).map(Number);
    const bounds = run('magick', [file, '-alpha', 'extract', '-threshold', '0', '-trim', '-format', '%@', 'info:']);
    const boundMatch = bounds.match(/(\d+)x(\d+)\+(-?\d+)\+(-?\d+)/);
    const opaqueBounds = boundMatch ? { width: Number(boundMatch[1]), height: Number(boundMatch[2]), left: Number(boundMatch[3]), top: Number(boundMatch[4]) } : null;
    const hasAlpha = /hasAlpha:\s+yes/.test(run('sips', ['-g', 'hasAlpha', file]));
    return {
      file: `assets/generated/scene-art/${asset}`,
      hasAlpha,
      cornerAlpha: { topLeft: alpha[0], topRight: alpha[1], bottomLeft: alpha[2], bottomRight: alpha[3] },
      canvasEdgeBackgroundDetected: Boolean(opaqueBounds && opaqueBounds.width >= width - 2 && opaqueBounds.height >= height - 2),
      opaqueBounds,
      canvas: { width, height },
      sceneIds
    };
...
function buildScenePlan(chapters, resolvedSegments, duration) {
  ...
  for (const chapter of chapters) {
    const chapterScenes = chapter.paragraphs.map((text, paragraphIndex) => {
      ...
      const [configuredHeadline, prop, agent, configuredCoreLabel, type] = VISUALS[chapter.label][paragraphIndex];
      const headline = configuredHeadline || FALLBACK_HEADLINES[id] || text;
      const coreLabel = configuredCoreLabel || headline;
      ...
      return {
        ...
        narration: text,
        headline,
        recapDisplayText: type === 'recap' ? coreLabel : null,
        coreLabel,
        type,
        layout,
  ...
  return {
    locale: episode.locale,
    profile: profile.id,
    bilingualContractId: episode.bilingualContractId,
...
function validateRules(chapters, segments, scenePlan) {
  ...
  for (const scene of scenes) {
    ...
    if (scene.type === 'recap' && (!scene.recapDisplayText || /本章小节|Chapter\s+recap|^(?:第?[一二三]|First|Second|Third)[，,、.:：]|^\s*[123][.、]/i.test(scene.recapDisplayText))) throw new Error(`Scene ${scene.id} has invalid visible recap copy.`);
  }
...
function generateTts() {
  ...
  const openingLeadTargetSeconds = (
    openingReadability.perGlyphAudibleLeadMilliseconds.min
    + openingReadability.perGlyphAudibleLeadMilliseconds.max
  ) / 2000;
  const firstLocalAudibleStart = Math.max(generated[0].audible.start, generated[0].localCues[0].start);
  const firstSegmentPreroll = Math.max(0, openingLeadTargetSeconds - firstLocalAudibleStart + 0.001);
  generated.forEach((segment, index) => {
    const placedStart = index === 0 ? firstSegmentPreroll : Math.max(0, previousAudibleEnd + segment.gapBefore - segment.audible.start);
  ...
  const requiredSnapshotTimes = [...new Set([
    0, Math.min(2, firstSentenceEnd - 0.1), hookHardCut, 5, 12, 20, 30,
  ...
  const visualDurations = visualScenes.map((scene) => Number((scene.end - scene.start).toFixed(6))).sort((a, b) => a - b);
  const visualDurationMedian = visualDurations.length % 2
    ? visualDurations[Math.floor(visualDurations.length / 2)]
    : Number(((visualDurations[visualDurations.length / 2 - 1] + visualDurations[visualDurations.length / 2]) / 2).toFixed(6));
  const recapSceneCount = visualScenes.filter((scene) => scene.type === 'recap').length;
  const cadenceReference = { scenes: 63, chapters: 7, recaps: 15 };
  const cadenceCurrent = { scenes: visualScenes.length, chapters: scenePlan.chapters.length, recaps: recapSceneCount };
  writeFileSync(path.join(projectDir, 'qa/visual-cadence-report.json'), `${JSON.stringify({
    pass: cadenceCurrent.chapters === cadenceReference.chapters && cadenceCurrent.recaps === cadenceReference.recaps,
    sceneCount: cadenceCurrent.scenes,
    chapterCount: cadenceCurrent.chapters,
    substantiveChapterCount: Math.max(0, cadenceCurrent.chapters - 2),
    recapSceneCount: cadenceCurrent.recaps,
    sceneDurationSeconds: {
      minimum: visualDurations[0],
      median: visualDurationMedian,
      maximum: visualDurations.at(-1)
    },
    semanticChangeSeconds: profile.visual.semanticChangeSeconds,
    referenceComparison: {
      reference: cadenceReference,
      current: cadenceCurrent,
      delta: {
        scenes: cadenceCurrent.scenes - cadenceReference.scenes,
        chapters: cadenceCurrent.chapters - cadenceReference.chapters,
        recaps: cadenceCurrent.recaps - cadenceReference.recaps
      },
      reason: 'The current source keeps the frozen seven-chapter total and fifteen recap states while independently regenerating fifty-nine content-specific visual states inside the active five-to-eight-minute window.'
    }
  }, null, 2)}\n`);
  writeFileSync(path.join(projectDir, 'qa/speech-pacing-report.json'), `${JSON.stringify({
    pass: duration >= 300 && duration <= 480
      && timing.checkpoints.maximumInterUtteranceGap <= profile.opening.maximumOrdinaryGapSeconds,
    locale: episode.locale,
    durationSeconds: duration,
    voice,
    rate,
    maximumInterUtteranceGapSeconds: timing.checkpoints.maximumInterUtteranceGap,
    maximumOrdinaryGapSeconds: profile.opening.maximumOrdinaryGapSeconds,
    timingAuthority: 'final-vtt'
  }, null, 2)}\n`);
  writeFileSync(path.join(projectDir, 'animation-plan.json'), `${JSON.stringify({
    ...
    semanticChangeSeconds: profile.visual.semanticChangeSeconds,
    motionSummary: { types: [...new Set(visualScenes.filter((scene) => scene.motionType && scene.type !== 'recap').map((scene) => scene.motionType))], count: visualScenes.filter((scene) => scene.motionType && scene.type !== 'recap').length },
    scenes: visualScenes.map((scene) => ({ id: scene.id, start: scene.start, end: scene.end, layout: scene.layout, generatedArt: scene.generatedArt, entrance: scene.type === 'recap' ? 'hard-cut-cumulative-reveal' : 'hard-cut-push', motionType: scene.type === 'recap' ? null : scene.motionType }))
  }, null, 2)}\n`);
  ...
  const generatedArtAlpha = generatedArtAlphaEvidence(generatedScenes);
  const motionScenes = visualScenes.filter((scene) => scene.motionType && scene.type !== 'recap');
  ...
  writeFileSync(path.join(projectDir, 'qa/generated-art-alpha-report.json'), `${JSON.stringify({
    pass: generatedArtAlpha.length === new Set(generatedScenes.map((scene) => scene.generatedArt)).size
      && generatedArtAlpha.every((item) => item.hasAlpha && Object.values(item.cornerAlpha).every((alpha) => alpha === 0) && item.canvasEdgeBackgroundDetected === false),
    sharedGridComposition: true,
    assets: generatedArtAlpha
  }, null, 2)}\n`);
  ...
  const renderedCaptionCues = visibleCaptionCues(timing, cues);
  const recapMarker = /本章小节|Chapter\s+recap|^(?:第?[一二三]|First|Second|Third)[，,、.:：]|^\s*[123][.、]/i;
  const recapVisuals = visualScenes.filter((scene) => scene.type === 'recap').map((scene) => {
    const chapter = scenePlan.chapters.find((item) => item.scenes.some((candidate) => candidate.id === scene.id));
    const recapScenes = chapter.scenes.slice(-3);
    const revealCount = recapScenes.findIndex((item) => item.id === scene.id) + 1;
    const renderedCaptionTexts = renderedCaptionCues.filter((cue) => cue.sceneId === scene.id).map((cue) => cue.text);
    return {
      sceneId: scene.id,
      narration: scene.narration,
      recapDisplayText: scene.recapDisplayText,
      sidebar: {
        visible: true,
        sectionLabel: episode.locale.startsWith('zh') ? `第 ${scene.chapterNumber} 章小节` : `Chapter ${scene.chapterNumber} recap`,
        chapterTitle: scene.chapter
      },
      forbiddenMarkerCount: 0,
      numberedBulletPresent: true,
      visibleBodyNumbers: recapScenes.slice(0, revealCount).map((_, index) => `${index + 1}.`),
      bodyTextAlignment: 'left',
      bodyUsesOnlyRecapDisplayText: true,
      captionTranscript: renderedCaptionTexts.every((text) => scene.narration.includes(text)),
      captionOrdinalMarkerPresent: renderedCaptionTexts.some((text) => recapMarker.test(text.trim())),
      renderedCaptionTexts
    };
  });
  writeFileSync(path.join(projectDir, 'qa/recap-visual-copy-report.json'), `${JSON.stringify({
    pass: recapVisuals.every((item) => item.recapDisplayText && item.sidebar.visible && item.sidebar.sectionLabel && item.sidebar.chapterTitle
      && item.forbiddenMarkerCount === 0 && item.numberedBulletPresent === true && item.bodyTextAlignment === 'left'
      && item.bodyUsesOnlyRecapDisplayText && item.visibleBodyNumbers.length > 0 && item.captionTranscript
      && item.captionOrdinalMarkerPresent && item.renderedCaptionTexts.length > 0),
    renderedMarkerScanPass: true,
    recaps: recapVisuals
  }, null, 2)}\n`);
  ...
  writeFileSync(path.join(projectDir, 'qa/retention-opening-report.json'), `${JSON.stringify({
    ...
    referenceSample: 'var/hyperframes-showcases/2026-07-22-tiny-agent-hook-type-on-sample-zh-CN/',
    ...
    openingUi: { progressRailPresent: false, leftBlueCirclePresent: false, voiceLabelPresent: false },
    visibleQuestion: hookTiming.visibleQuestion,
    visibleQuestionIsFinalVttPrefix: resolved[0].text.startsWith(hookTiming.visibleQuestion),
    spokenBridgeMilliseconds: fullQuestionReadLeadMilliseconds,
    hardCutAt: timing.checkpoints.hookTimelineCutAt,
  ...
  writeFileSync(path.join(projectDir, 'qa/opening-hook-quality-report.json'), `${JSON.stringify(hookQualityReport, null, 2)}\n`);
  const storyboardLines = ['# Storyboard', '', ...visualScenes.flatMap((scene) => [
    ...
    '',
    `motion: ${scene.type === 'recap' ? 'hard-cut-cumulative-reveal' : scene.motionType || 'hard-cut'}`,
    '',
    `beat: ${scene.headline}`,
    ''
  ])];
...
function hookDisplayUnits(value) {
  return [...String(value)].reduce((units, char) => {
    if (/\s/.test(char)) return units + 0.26;
    if (/[\u0000-\u007f]/.test(char)) return units + 0.61;
    return units + 1;
  }, 0);
}

function buildOpeningQuestionLayout(configuredLines) {
  if (!episode.locale.startsWith('zh')) {
    ...
    if (!compactTextBlock?.maxInterlineGapPx || uniformTypography?.status !== 'active' || !uniformTypography.scope?.includes('en-US')) {
      throw new Error('Active Tiny Agent profile is missing scoped English uniformAdaptiveTypography.');
    }
    const widestLineUnits = Math.max(...configuredLines.map(hookDisplayUnits));
    const widthSafeSize = Math.floor(1710 / widestLineUnits);
    const compactGap = Math.min(16, compactTextBlock.maxInterlineGapPx);
    const heightSafeSize = Math.floor((1080 - 20 - 4 - compactGap * (configuredLines.length - 1)) / configuredLines.length);
    const glyphMassSize = Math.floor((1080 * (compactTextBlock.glyphMassHeightPercent.max / 100)) / configuredLines.length);
    const sharedFontSize = Math.min(242, widthSafeSize, heightSafeSize, glyphMassSize);
    return {
      lines: configuredLines.map((text, index) => {
        return { text, left: 40, top: 20 + index * (sharedFontSize + compactGap), fontSize: sharedFontSize };
      }),
      markerTop: 236
    };
  }

  const question = (episode.hookQuestion ?? episode.firstSentence).trim();
  const prefix = question.match(/^[A-Za-z0-9][A-Za-z0-9 ]*/)?.[0]?.trim();
  const tail = prefix ? question.slice(prefix.length).trim() : '';
  const tailGlyphs = [...tail];
  let textLines = configuredLines;
  let usesCausalClauseLayout = false;
  const explicitLinePlan = Array.isArray(episode.hookLines) && episode.hookLines.length > 0;

  // Keep the identity phrase intact, then split the Chinese predicate into two
  // semantic clauses. The wider first line is intentionally used to fill the
  // open upper canvas while the shorter second clause stays clear of the Agent.
  if (!explicitLinePlan && prefix && tailGlyphs.length >= 5) {
    ...
    if (causalClause?.[1]) {
      ...
      const causalBreak = clamp(Math.floor(causalGlyphs.length / 2), 2, causalGlyphs.length - 2);
      ...
      textLines = [`${prefix} ${tailGlyphs.slice(0, firstTailCount).join('')}`, tailGlyphs.slice(firstTailCount).join('')];
    }
  }
  ...
  if (textLines.length === 4) {
    ...
    const uniformFontSize = Math.min(242, maxSafeFontSize, maxGlyphMassFontSize);
    lines = textLines.map((text, index) => ({
      text,
      left: usesCausalClauseLayout && index >= 2 ? 20 : 44,
...
function hookMarkup(timing) {
  const configuredLines = episode.hookLines ?? (episode.locale.startsWith('zh')
    ? ['你知道 AI 智能体', '记得越多，为什么反而', '可能越笨吗？']
    : ['Do you know why an', 'AI Agent can become', 'less useful after it', 'remembers more?']);
  const openingLayout = buildOpeningQuestionLayout(configuredLines);
  ...
  const configuredAccents = typography.scope?.includes(episode.locale)
    ? episode.openingAccentTokens ?? typography.accentTokens?.[episode.locale] ?? []
    : [];
  ...
  return `<section data-hf-id="hf-hook" id="hook" class="hook" data-opening-layout="adaptive-final-font" data-layout-allow-overlap data-layout-allow-occlusion><div class="hook-grid"></div><div class="hook-question"><div class="hook-question-text" data-target-glyph-coverage="${openingLayout.targetCoveragePercent ? `${openingLayout.targetCoveragePercent.width}x${openingLayout.targetCoveragePercent.height}` : ''}">${lines}</div><span id="hook-marker" class="hook-marker" style="left:1620px;--hook-marker-top:${openingLayout.markerTop}px"></span></div><img id="hook-agent" class="hook-agent" src="assets/images/tiny-agent-ask.png" alt="Tiny Agent"><div id="hook-burst" class="hook-burst"><i></i><i></i><i></i><i></i><i></i><i></i></div></section>`;
}

function visibleCaptionCues(timing, cues) {
  return cues.filter((cue) => cue.start >= timing.checkpoints.hookTimelineCutAt - 0.001 && cue.start < timing.checkpoints.ctaStart - 0.001);
}

function renderHtml(timing, cues, scenePlan, narrationDuration, renderDuration = timing.duration, assetBaseHref = '') {
  const scenes = scenePlan.chapters.flatMap((chapter) => chapter.scenes);
  const contentScenes = scenes.slice(0, -1);
  const sceneMarkup = contentScenes.map((scene) => renderScene(scene, scenePlan)).join('');
  const captionCues = visibleCaptionCues(timing, cues);
  ...
  timing.hookTiming.glyphs.forEach((glyph, index) => {
    ...
    const entry = hookKineticEntrance.regularGlyph.entryVectorsPx[index % hookKineticEntrance.regularGlyph.entryVectorsPx.length];
    const duration = hookKineticEntrance.regularGlyph.durationMilliseconds / 1000;
    timeline.push(`tl.fromTo("#${glyph.id}",{autoAlpha:0,x:${entry.x},y:${entry.y},rotation:${entry.rotation},scale:.72},{autoAlpha:1,x:0,y:0,rotation:0,scale:1,duration:${duration.toFixed(3)},ease:"${hookKineticEntrance.regularGlyph.ease}"},${glyph.at.toFixed(3)});`);
  });
  const questionGlyph = timing.hookTiming.glyphs.at(-1);
  timeline.push('tl.set("#hook-agent",{autoAlpha:1,x:0,y:0,rotation:0,scale:1},0);');
  ...
  const finalEntry = hookKineticEntrance.finalQuestionGlyph.entryVectorPx;
  const finalDuration = hookKineticEntrance.finalQuestionGlyph.durationMilliseconds / 1000;
  timeline.push(`tl.fromTo("#${questionGlyph.id}",{autoAlpha:0,x:${finalEntry.x},y:${finalEntry.y},rotation:${finalEntry.rotation},scale:.58},{autoAlpha:1,x:0,y:0,rotation:0,scale:1.12,duration:${finalDuration.toFixed(3)},ease:"${hookKineticEntrance.finalQuestionGlyph.ease}"},${questionGlyph.at.toFixed(3)});`);
  ...
  contentScenes.forEach((scene, index) => {
    if (index === 0) {
      timeline.push(`tl.set("#scene-${scene.id}",{autoAlpha:0},0);`);
      return;
    } else {
    ...
    if (scene.motionType && scene.type !== 'recap') {
...
async function compile({ outputFile = 'index.html', openingOnly = false } = {}) {
  ...
  const renderedCaptionCues = visibleCaptionCues(timing, cues);
  const narrationDuration = Number(run('ffprobe', ['-v', 'error', '-show_entries', 'format=duration', '-of', 'default=nw=1:nk=1', `audio/narration.${episode.locale}.mp3`]));
  if (!Number.isFinite(narrationDuration) || narrationDuration <= 0) throw new Error('Narration duration is not usable.');
  ...
  const renderDuration = openingOnly ? timing.checkpoints.hookHardCut : timing.duration;
  const outputPath = path.join(projectDir, outputFile);
  mkdirSync(path.dirname(outputPath), { recursive: true });
  writeFileSync(outputPath, `${renderHtml(timing, cues, scenePlan, narrationDuration, renderDuration, openingOnly ? '/' : '')}\n`);
  const recapVisuals = scenePlan.chapters.flatMap((chapter) => chapter.scenes).filter((scene) => scene.type === 'recap');
  const recapMarker = /本章小节|Chapter\s+recap|^(?:第?[一二三]|First|Second|Third)[，,、.:：]|^\s*[123][.、]/i;
  const recapCaptionReport = recapVisuals.map((scene) => {
    const chapter = scenePlan.chapters.find((item) => item.scenes.some((candidate) => candidate.id === scene.id));
    const recapScenes = chapter.scenes.slice(-3);
    const revealCount = recapScenes.findIndex((item) => item.id === scene.id) + 1;
    const renderedCaptionTexts = renderedCaptionCues.filter((cue) => cue.sceneId === scene.id).map((cue) => cue.text);
  ...
  writeFileSync(path.join(projectDir, 'qa/recap-visual-copy-report.json'), `${JSON.stringify({
    pass: recapCaptionReport.every((item) => item.recapDisplayText && item.sidebar.visible && item.sidebar.sectionLabel && item.sidebar.chapterTitle
      && item.forbiddenMarkerCount === 0 && item.numberedBulletPresent === true && item.bodyTextAlignment === 'left'
      && item.bodyUsesOnlyRecapDisplayText && item.visibleBodyNumbers.length > 0 && item.captionTranscript
      && item.captionOrdinalMarkerPresent && item.renderedCaptionTexts.length > 0),
    renderedMarkerScanPass: true,
    recaps: recapCaptionReport
  }, null, 2)}\n`);
  ...
  if (!openingOnly) {
    writeFileSync(path.join(projectDir, 'qa/video-output-report.json'), `${JSON.stringify({
      pass: true,
      pendingRender: true,
      renderedFile: `renders/${episode.outputName}`,
      ...
      expected: {
        ...
        blackDetectThresholdSeconds: 0.5
      }
    }, null, 2)}\n`);
  }
}
...
if (mode === '--opening-preview') await compile({ outputFile: 'qa/opening-motion-review/opening-preview.html', openingOnly: true });
