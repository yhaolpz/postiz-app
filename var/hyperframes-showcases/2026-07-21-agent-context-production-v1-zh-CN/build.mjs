...
const fps = 30;
const tailPad = 0.3;
const outputName = 'ai-agent-context-production-v1.zh-CN.mp4';
...
const VISUALS = {
  '前言': [
    ...
    ['AI 智能体每一步只保留四件事', 'task-list', 'verify-front', '目标 · 约束 · 证据 · 当前状态', 'list'],
    ['AI 智能体需要最小充分上下文', 'target', 'present-left', '支持下一步行动', 'generic']
  ],
  '注意力预算': [
    ['AI 智能体看见的是完整上下文', 'document-stack', 'read-front', '指令 · 工具 · 历史 · 结果', 'generic'],
    ['窗口更大，不代表注意力免费', 'trend-down', 'reason-front', '新增内容的收益会下降', 'meter'],
    ['规则写进去了，也可能失去优先级', 'warning', 'alert-front', '过期计划会稀释重点', 'generic'],
    ['只保留会改变下一步判断的信息', 'target', 'evaluate-front', '目标 · 约束 · 证据 · 状态', 'list'],
    ['上下文必须随着任务持续刷新', 'summary', 'monitor-left', '新证据进入，旧轨迹退出', 'generic'],
    ['最小充分，不等于字数最少', 'ruler', 'compare-front', '足以支持预期行为', 'generic'],
    ['1. 把上下文当作注意力预算', 'timer', 'plan-front', '有限资源', 'recap'],
    ['2. 更多 token 可能稀释重点', 'document-stack', 'evaluate-front', '更多 ≠ 更强', 'recap'],
    ['3. 随任务持续刷新上下文', 'summary', 'verify-front', '持续更新', 'recap']
  ],
  '高信号输入': [
    ['指令要落在合适的抽象层级', 'ruler', 'reason-front', '不硬编码，也不空泛', 'generic'],
    ['结构有帮助，优先级更重要', 'clipboard', 'plan-front', '目标 · 边界 · 判断原则', 'list'],
    ['工具是 AI 智能体与环境的契约', 'tool', 'receive-tool', '名称 · 参数 · 错误 · 返回', 'list'],
    ['重叠工具会制造模糊决策', 'compare', 'evaluate-front', '职责单一，结果紧凑', 'generic'],
    ['少量典型示例胜过边缘堆积', 'skill-card', 'present-left', '主要模式 + 关键边界', 'generic'],
    ['每段高信号都必须承担工作', 'evidence', 'verify-front', '结果 · 约束 · 工具 · 证据', 'list'],
    ['1. 指令层级要恰到好处', 'ruler', 'plan-front', '明确但不僵化', 'recap'],
    ['2. 工具清楚并减少重叠', 'tool', 'evaluate-front', '最小可用工具集', 'recap'],
    ['3. 用典型示例展示模式', 'skill-card', 'present-left', '少量 · 多样 · 典型', 'recap']
  ],
  '按需检索': [
    ['别在开始前塞入全部文档', 'document-stack', 'blocked-front', '预算提前被花掉', 'generic'],
    ['上下文里先保留一张地图', 'route', 'plan-front', '路径 · 查询 · 标识 · 时间戳', 'list'],
    ['让 AI 智能体逐层建立理解', 'search', 'search', '渐进式披露', 'generic'],
    ['Claude Code 只打开目标片段', 'code-file', 'read-front', '搜索后再读取', 'generic'],
    ['按需检索也有真实成本', 'timer', 'evaluate-front', '更慢，也可能走错方向', 'generic'],
    ['稳定规则预载，细节按需打开', 'route', 'present-left', '混合策略', 'generic'],
    ['1. 保留轻量引用', 'citation', 'plan-front', '不要预载全部正文', 'recap'],
    ['2. 相关时再发现细节', 'search', 'search', '需要时打开', 'recap'],
    ['3. 平衡聚焦与检索成本', 'compare', 'evaluate-front', '收益必须超过成本', 'recap']
  ],
  '长任务记忆': [
    ['长任务会不断填满上下文窗口', 'progress', 'monitor-left', '动作 · 观察 · 错误 · 决定', 'generic'],
    ['用压缩摘要开启新窗口', 'summary', 'store-memory', '决定 · 约束 · 证据 · 下一步', 'list'],
    ['压缩先追求高召回', 'shield', 'verify-front', '宁可多带，不能漏掉关键约束', 'generic'],
    ['把工作状态写到窗口之外', 'note', 'write-front', '结构化外部笔记', 'generic'],
    ['专门 Agent 隔离深度探索', 'parallel-agents', 'delegate-front', '主 Agent 保留高层计划', 'generic'],
    ...
    ['3. 专门 Agent 只用于深挖', 'parallel-agents', 'delegate-front', '价值要超过协调成本', 'recap']
  ],
  '上下文包': [
    ['六部分高信号上下文包', 'package', 'handoff-left', '紧凑、完整、可检查', 'pack'],
    ['第一部分：结果目标', 'target', 'present-left', '目标 · 使用者 · 交付物 · 验收', 'list'],
    ['第二部分：运行规则', 'shield', 'alert-front', '约束 · 权限 · 风险 · 停止', 'list'],
    ['第三部分：工具地图与典型示例', 'tool', 'receive-tool', '使用条件 · 输入 · 紧凑输出', 'list'],
    ['第四部分：检索索引', 'citation', 'search', '入口 · 用途 · 打开条件', 'list'],
    ['第五部分：工作状态', 'progress', 'monitor-left', '重点 · 决定 · 未知 · 下一步', 'list'],
    ['1. 用目标和规则锚定智能体', 'target', 'plan-front', '结果目标 + 运行规则', 'recap'],
    ['2. 用地图、示例和索引指导行动', 'route', 'present-left', '工具 · 示例 · 检索', 'recap'],
...
function generateTts() {
  ...
  for (const segment of segments) {
    ...
    if (!existsSync(audioFile) || !existsSync(vttFile)) {
...
function renderHtml(timing, cues, scenePlan) {
  ...
  const sceneMarkup = contentScenes.map((scene, index) => {
    const side = index % 2 === 0 ? 'agent-left' : 'agent-right';
  ...
  contentScenes.forEach((scene, index) => {
    ...
    timeline.push(`tl.fromTo("#scene-${scene.id} h1",{x:${index % 2 ? 46 : -46}},{x:0,duration:.36,ease:"power3.out"},${scene.start.toFixed(3)});`);
    timeline.push(`tl.fromTo("#scene-${scene.id} .agent",{y:12},{y:0,duration:.4,ease:"power2.out"},${scene.start.toFixed(3)});`);
    timeline.push(`tl.fromTo("#scene-${scene.id} .core-card",{x:${index % 2 ? -34 : 34}},{x:0,duration:.4,ease:"power2.out"},${scene.start.toFixed(3)});`);
