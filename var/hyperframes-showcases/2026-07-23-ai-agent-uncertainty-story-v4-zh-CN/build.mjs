...
function generateTts() {
  ...
  writeFileSync(path.join(projectDir, 'qa/recap-report.json'), `${JSON.stringify({
    pass: scenePlan.chapters.slice(1, -1).every((chapter) => chapter.scenes.every((scene) => scene.type !== 'recap')),
    mode: 'natural-transition-no-formal-recap',
    formalRecapSceneCount: visualScenes.filter((scene) => scene.type === 'recap').length,
    transitions: scenePlan.chapters.slice(1, -2).map((chapter) => ({
      label: chapter.label,
  ...
  writeFileSync(path.join(projectDir, 'qa/recap-spoken-prefix-report.json'), `${JSON.stringify({
    pass: !visualScenes.some((scene) => /本章小节|第\\s*[一二三四五六七八九十0-9]+\\s*章小节/.test(scene.narration)),
    requiredPrefix: null,
