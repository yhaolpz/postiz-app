...
const sourcePath = path.join(
  repoRoot,
  'scripts/ai-video-pipeline/content-plans/2026-07-building-effective-agents-longform.zh-CN.md',
);
...
function renderPhaseV2(scene, index) {
  if (scene.recap) {
    ...
    const summaryTitle = scene.finalSummary ? '从简单开始\n用结果决定' : scene.chapter;
    const summaryCta = scene.finalSummary ? `
          <div class="summary-cta"><strong>Tiny Agent</strong><span>更少时间，理解真正重要的 AI / Agent 知识</span></div>` : '';
    return `
...
<html lang="zh-CN">
  ...
  <body>
    <div data-hf-id="hf-main-root" id="root" data-composition-id="main" data-start="0" data-duration="${duration.toFixed(3)}" data-width="1920" data-height="1080">
      ...
      <header data-hf-id="hf-masthead" class="masthead"><div class="brand">Tiny Agent</div><h1 class="title">别急着做多 Agent</h1></header>
      <div data-hf-id="hf-caption-band" class="caption-band">${captionMarkup}
      </div>
      <nav data-hf-id="hf-chapter-progress" class="chapter-progress" aria-label="视频章节进度">${progressMarkup}
      </nav>
      <audio data-hf-id="hf-narration" id="narration" class="clip" data-start="0" data-duration="${duration.toFixed(3)}" data-track-index="10" data-media-start="0" data-volume="1" src="assets/audio/narration.zh.normalized.mp3"></audio>
    </div>
    <script>
      window.__timelines = window.__timelines || {};
      const tl = gsap.timeline({ paused: true });
...
async function compile() {
  ...
  await writeFile(path.join(projectRoot, 'summary.json'), `${JSON.stringify({
    title: '别急着做多 Agent：如何选择真正有效的 Agent 架构',
    source: sourcePath,
    language: 'zh-CN',
    ...
    output: { file: 'renders/building-effective-agents-longform.zh-CN.mp4', status: 'pending' },
  }, null, 2)}\n`);
