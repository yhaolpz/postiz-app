...
const sourcePath = path.join(projectRoot, 'SCRIPT.en-US.md');
const audioDir = path.join(projectRoot, 'assets/audio');
const audioSegmentsDir = path.join(audioDir, 'retention-segments');
const vttSegmentsDir = path.join(audioDir, 'retention-vtt');
...
const visualBalanceTarget = { x: 960, y: 450 };
...
function detectAudibleRange(file, duration) {
  const result = spawnSync('ffmpeg', [
    '-hide_banner', '-nostats', '-loglevel', 'info', '-vn', '-i', file,
    '-af', 'silencedetect=noise=-50dB:d=0.05', '-f', 'null', '-',
  ], { encoding: 'utf8' });
  if (result.status !== 0) throw new Error(`Audible-range detection failed for ${file}: ${result.stderr}`);
  const starts = [...result.stderr.matchAll(/silence_start:\s*([0-9.]+)/g)].map((match) => Number(match[1]));
  ...
  const end = Number.isFinite(trailingStart) && trailingEnd >= duration - 0.05 ? trailingStart : duration;
  return { start: Number(start.toFixed(3)), end: Number(Math.max(start, end).toFixed(3)) };
}

function splitParagraphIntoUtterances(paragraph) {
  return (paragraph.match(/[^.!?]+[.!?]+|[^.!?]+$/g) || [paragraph])
    .map((utterance) => utterance.trim())
    .filter(Boolean);
}

function gapBeforeUtterance({ chapterIndex, paragraphIndex, utteranceIndex, text }) {
  if (chapterIndex === 0 && paragraphIndex === 0 && utteranceIndex === 0) return 0;
  if (chapterIndex === 0 && paragraphIndex === 1 && utteranceIndex === 0) return 0.45;
  if (utteranceIndex === 0 && paragraphIndex === 0) return 0.32;
  if (utteranceIndex === 0) return 0.22;
  if (/[?]$/.test(text)) return 0.24;
  return 0.16;
}
      ...
      if (scene.recap) {
        scene.summaryPointParagraphs.forEach((paragraphNumber, pointIndex) => {
          const spokenPoint = chapter.paragraphs[paragraphNumber - 1]
            .replace(/^(First|Second|Third)[，,]\s*/i, '')
            .replace(/[。.]$/, '');
          if (normalizeAlignmentText(spokenPoint).toLowerCase() !== normalizeAlignmentText(plannedChapter.summary[pointIndex]).toLowerCase()) {
...
function splitCaptionText(text, limit = 72) {
  if ([...text].length <= limit) return [text];
  const output = [];
  let current = '';

  for (const token of text.split(/\s+/)) {
    const pieces = [...token].length > limit ? splitLongToken(token, limit) : [token];
...
function normalizeCaptionTimeline(captions) {
  for (let index = 1; index < captions.length; index += 1) {
    const previous = captions[index - 1];
    const current = captions[index];
    current.start = Math.max(current.start, previous.start + 0.08);
    previous.end = Math.max(previous.start + 0.04, Math.min(previous.end, current.start));
    current.end = Math.max(current.start + 0.08, current.end);
  }
}
...
function applyRetentionScenePlan(plan, animationPlan) {
  plan.retentionPolicy = {
    version: 'retention-v2-large-simple',
    activeCharactersPerBodyScene: 1,
    maximumCorePropsPerBodyScene: 2,
    minimumAgentFrameHeight: 500,
  };
  plan.chapters.forEach((chapter) => {
    chapter.scenes.forEach((scene, sceneIndex) => {
      if (scene.recap || scene.props.length <= 2) return;
      const original = scene.props;
      scene.props = scene.layout === 'comparison'
        ? [original[0], original[Math.floor(original.length / 2)]]
        : [original[0], original.at(-1)];
      scene.flow = scene.layout === 'process' ? 'chain' : 'cluster';
      const sceneId = `c${String(plan.chapters.indexOf(chapter) + 1).padStart(2, '0')}-s${String(sceneIndex + 1).padStart(2, '0')}`;
      if (animationPlan.scenes[sceneId]) animationPlan.scenes[sceneId].beats[0].props = scene.props.map((_, index) => index);
    });
  });
  plan.chapters[0].scenes[0].callout = 'Why AI Agents Forget Long Tasks';
  plan.chapters[0].scenes[1].callout = 'Context Is a Finite Attention Budget';
  plan.chapters[0].scenes[2].callout = 'Six-Part AI Agent Context Pack';
  animationPlan.scenes['c01-s01'].beats[0].trigger = 'Why does an AI Agent forget';
  animationPlan.scenes['c01-s02'].beats[0].trigger = "Anthropic's context-engineering work";
  animationPlan.scenes['c01-s03'].beats[0].trigger = 'six-part high-signal context pack';
}

async function prepare() {
  const chapters = parseSource(await readFile(sourcePath, 'utf8'));
  await mkdir(audioDir, { recursive: true });
  await mkdir(audioSegmentsDir, { recursive: true });
  await mkdir(vttSegmentsDir, { recursive: true });
  ...
  applyRetentionScenePlan(plan, animationPlan);
  await writeFile(planPath, `${JSON.stringify(plan, null, 2)}\n`);
  await writeFile(animationPlanPath, `${JSON.stringify(animationPlan, null, 2)}\n`);
...
async function generateTts() {
  ...
  for (const chapter of chapters) {
    const chapterNumber = String(chapter.index + 1).padStart(2, '0');
    const utterances = chapter.paragraphs.flatMap((paragraph, paragraphIndex) => (
      splitParagraphIntoUtterances(paragraph).map((text, utteranceIndex) => ({ paragraphIndex, utteranceIndex, text }))
    ));
    const generated = [];
    for (const utterance of utterances) {
      const id = `chapter-${chapterNumber}-p${String(utterance.paragraphIndex + 1).padStart(2, '0')}-u${String(utterance.utteranceIndex + 1).padStart(2, '0')}`;
      const media = path.join(audioSegmentsDir, `${id}.mp3`);
      const subtitles = path.join(vttSegmentsDir, `${id}.vtt`);
      let reusable = false;
      try {
        await access(media);
        await access(subtitles);
        const cachedCues = parseVtt(await readFile(subtitles, 'utf8'));
        reusable = normalizeAlignmentText(cachedCues.map((cue) => cue.text).join('')) === normalizeAlignmentText(utterance.text);
      } catch {
        reusable = false;
      }
      if (!reusable) {
        execFileSync('uvx', [
          'edge-tts', '--voice', voice, '--rate', rate, '--text', utterance.text,
          '--write-media', media, '--write-subtitles', subtitles,
        ], { stdio: 'inherit' });
      }
      const duration = probeDuration(media);
      generated.push({ ...utterance, id, media, subtitles, duration, audible: detectAudibleRange(media, duration) });
    }

    let previousAudibleEnd = 0;
    const placed = generated.map((utterance, index) => {
      const gap = gapBeforeUtterance({ chapterIndex: chapter.index, ...utterance });
      const audioStart = index === 0 ? 0 : Math.max(0, previousAudibleEnd + gap - utterance.audible.start);
      const spokenStart = audioStart + utterance.audible.start;
      const spokenEnd = audioStart + utterance.audible.end;
      const actualGap = index === 0 ? spokenStart : spokenStart - previousAudibleEnd;
      previousAudibleEnd = spokenEnd;
      return { ...utterance, gap, audioStart, spokenStart, spokenEnd, actualGap };
    });
    const chapterDuration = Number((Math.ceil((previousAudibleEnd + 0.24) * 30) / 30).toFixed(6));
    const ffmpegArgs = ['-y', '-f', 'lavfi', '-t', String(chapterDuration), '-i', 'anullsrc=r=48000:cl=stereo'];
    placed.forEach((utterance) => ffmpegArgs.push('-i', utterance.media));
    const delayed = placed.map((utterance, index) => `[${index + 1}:a]aresample=48000,adelay=${Math.round(utterance.audioStart * 1000)}|${Math.round(utterance.audioStart * 1000)}[s${index}]`);
    const mixInputs = ['[0:a]', ...placed.map((_, index) => `[s${index}]`)].join('');
    ffmpegArgs.push(
      '-filter_complex', `${delayed.join(';')};${mixInputs}amix=inputs=${placed.length + 1}:duration=first:dropout_transition=0:normalize=0[a]`,
      '-map', '[a]', '-t', String(chapterDuration), '-c:a', 'libmp3lame', '-b:a', '192k', path.join(audioDir, `chapter-${chapterNumber}.mp3`),
    );
    execFileSync('ffmpeg', ffmpegArgs, { stdio: 'inherit' });

    const chapterCues = [];
    for (const utterance of placed) {
      const localCues = parseVtt(await readFile(utterance.subtitles, 'utf8'));
      chapterCues.push(...localCues.map((cue) => ({
        start: utterance.audioStart + cue.start,
        end: utterance.audioStart + cue.end,
        text: cue.text,
      })));
    }
    const vtt = ['WEBVTT', '', ...chapterCues.flatMap((cue, index) => [
      String(index + 1),
      `${secondsToTimestamp(cue.start)} --> ${secondsToTimestamp(cue.end)}`,
      cue.text,
      '',
    ])].join('\n');
    await writeFile(path.join(audioDir, `chapter-${chapterNumber}.vtt`), `${vtt}\n`);
    report.push({
      chapter: chapter.label,
      duration: chapterDuration,
      utterances: placed.map(({ id, text, gap, actualGap, audioStart, spokenStart, spokenEnd }) => ({
        id, text, desiredGapBefore: gap, actualGapBefore: Number(actualGap.toFixed(3)),
        audioStart: Number(audioStart.toFixed(3)), spokenStart: Number(spokenStart.toFixed(3)), spokenEnd: Number(spokenEnd.toFixed(3)),
      })),
    });
  }
  await mkdir(path.join(projectRoot, 'qa'), { recursive: true });
  await writeFile(path.join(projectRoot, 'qa/utterance-timing-report.json'), `${JSON.stringify({
    policy: 'measured-audible-bounds-v2',
    voice,
    rate,
    maximumInterUtteranceGap: Math.max(...report.flatMap((chapter) => chapter.utterances.slice(1).map((item) => item.actualGapBefore))),
    chapters: report,
  }, null, 2)}\n`);
  await rm(path.join(audioDir, 'narration.en.normalized.mp3'), { force: true });
  process.stdout.write(`generated tight-cut TTS and VTT for ${chapters.length} chapters at ${rate}\n`);
}
...
async function mergeAudio(chapters) {
  ...
  const output = path.join(audioDir, 'narration.en.normalized.mp3');
  try {
    await access(output);
    return output;
  } catch {
    // Rebuild after TTS invalidates the normalized narration.
  }
  execFileSync('ffmpeg', [
    '-y', ...inputs,
...
function balancedClusterPropPositions(count, region) {
  if (count === 0) return [];
  if (count === 1) return [propPositionFromCenter(region.centerX, region.centerY, 320)];
  if (count === 2) {
    const size = 280;
    const separation = Math.min(430, region.width * 0.52);
    return [
...
function chainPropPositions(count, region) {
  ...
  const size = count <= 2 ? 300 : Math.max(130, Math.min(205, (region.width - (count - 1) * gap) / count - 48));
...
function comparisonPropPositions(count) {
  ...
  return [
    ...comparisonPanelPropPositions(leftCount, 480),
    ...comparisonPanelPropPositions(rightCount, 1090),
  ];
}
...
function validateComparisonPropPositions(scene) {
  ...
  groups.forEach((group, groupIndex) => {
    ...
    const panelCenter = groupIndex === 0 ? 825 : 1435;
...
const propRegions = {
  ...
  process: { centerX: 1240, centerY: 500, width: 880, height: 460 },
...
const layoutGeometry = {
  stage: {
    human: { x: 68, y: 330, width: 1, height: 1 },
    agent: { x: 70, y: 300, width: 430, height: 520 },
    callout: { x: 1000, y: 145, width: 720, align: 'center' },
  },
  process: {
    human: { x: 56, y: 470, width: 1, height: 1 },
    agent: { x: 60, y: 300, width: 420, height: 520 },
    callout: { x: 350, y: 132, width: 1220, align: 'center' },
  },
  grid: {
    human: { x: 68, y: 395, width: 1, height: 1 },
    agent: { x: 60, y: 300, width: 420, height: 520 },
    callout: { x: 420, y: 132, width: 1080, align: 'center' },
  },
  comparison: {
    human: { x: 56, y: 485, width: 1, height: 1 },
    agent: { x: 60, y: 300, width: 390, height: 500 },
    callout: { x: 420, y: 132, width: 1080, align: 'center' },
  },
  hero: {
    human: { x: 74, y: 430, width: 1, height: 1 },
    agent: { x: 70, y: 300, width: 430, height: 520 },
    callout: { x: 420, y: 170, width: 1080, align: 'center' },
  },
  focus: {
    human: { x: 60, y: 440, width: 1, height: 1 },
    agent: { x: 60, y: 300, width: 430, height: 520 },
...
function calloutTextMetrics(scene, frame) {
  ...
  const fontSize = [...scene.callout].length > 34 ? longFontSize : baseFontSize;
  const estimatedWidth = Math.max(fontSize * 2, [...scene.callout].length * fontSize * 0.58);
...
function validateTitleContentSeparation(scene) {
  ...
  const actorBoxes = [spriteVisibleBox(geometry.agent, agentBoundsById.get(scene.agent), { width: 384, height: 512 })];
  const props = scene.props.map(normalizeProp);
...
function sceneContentBounds(scene) {
  ...
  if (scene.layout === 'comparison') {
    boxes.push({ x: 480, y: 228, width: 690, height: 540 });
    boxes.push({ x: 1070, y: 228, width: 690, height: 540 });
  }
  return {
...
function renderPhaseV2(scene, index) {
  if (scene.recap) {
    const summaryLabel = scene.finalSummary ? 'Summary' : `Chapter ${scene.chapterNumber} recap`;
    const summaryTitle = scene.finalSummary ? 'AI Agent Context\nKeep the signal, lose the noise' : scene.chapter;
    const summaryCta = '';
    return `
  ...
  const chapterIntro = scene.showIntro ? `
        <div data-hf-id="hf-v2-${scene.id}-chapter-intro" class="chapter-intro">
          ...
          <div class="intro-kicker">Chapter ${scene.chapterNumber}</div>
      ...
      <section data-hf-id="hf-v2-phase-${scene.id}" id="v2-phase-${scene.id}" class="phase layout-${escapeHtml(scene.layout)} variant-${escapeHtml(scene.visualVariant || 'standard')}${scene.showIntro ? ' has-chapter-intro' : ''}${index === 0 ? ' is-first' : ''}" style="z-index:${index + 1}" data-chapter="${escapeHtml(scene.chapter)}" data-layout-allow-overlap data-layout-allow-overflow data-layout-allow-occlusion>
        <div class="scene-content-shell">
          <div class="balance-layer" style="transform:translate(${scene.balanceShift?.x || 0}px,${scene.balanceShift?.y || 0}px)">
            ...
            <div data-hf-id="hf-v2-${scene.id}-callout" class="callout${[...scene.callout].length > 34 ? ' callout-long' : ''} accent-${escapeHtml(scene.accent || 'blue')}" style="${calloutStyle(geometry.callout)}">${escapeHtml(scene.callout)}</div>
            <div class="prop-zone">${lines}${items}</div>
          </div>
        </div>
${chapterIntro}
...
function addSceneEntranceV2(timeline, scene, timing) {
  ...
  const { primeAt, contentAt, agentAt } = timing;
  const setAtPrime = (target, values) => primeAt === 0
    ? `gsap.set("${selector} ${target}", ${values});`
    : `tl.set("${selector} ${target}", ${values}, ${primeAt.toFixed(3)});`;
  timeline.push(`      ${setAtPrime('.agent-actor', '{ y: 10, opacity: 0 }')}
  ...
  if (scene.layout === 'hero') {
    ...
    timeline.push(`      ${setAtPrime('.callout', `{ x: ${calloutX}, opacity: 0 }`)}`);
    ...
    timeline.push(`      ${setAtPrime('.callout', '{ scale: 0.96, opacity: 0 }')}`);
    ...
    timeline.push(`      ${setAtPrime('.compare-panel-left', '{ clipPath: "inset(0 100% 0 0)" }')}`);
    timeline.push(`      ${setAtPrime('.compare-panel-right', '{ clipPath: "inset(0 0 0 100%)" }')}`);
    timeline.push(`      ${setAtPrime('.callout', '{ y: -12, opacity: 0 }')}`);
    ...
    <template id="story-template">
      <div data-hf-id="hf-v2-story-root" id="v2-story-root" data-composition-id="story" data-width="1920" data-height="1080" data-start="0" data-duration="${duration.toFixed(3)}">
        <style>
          ...
          #v2-story-root .prop-item span { position:relative; z-index:4; display:block; max-width:calc(var(--prop-size) + 44px); margin:8px auto 0; color:#47514e; font-size:32px; font-weight:850; line-height:1.2; letter-spacing:0; white-space:nowrap; }
          ...
          #v2-story-root .compare-panel-left { left:480px; border-top-color:#f5c84b; }
          #v2-story-root .compare-panel-right { right:160px; border-top-color:#1597ea; }
...
function renderIndex({ duration, narrationDuration, chapters, captions, scenes }) {
  ...
  const progressMarkup = chapters.map((chapter, index) => `
        <div data-hf-id="hf-progress-segment-${index + 1}" class="progress-segment" style="--chapter-weight:${chapter.duration.toFixed(3)}">
          <div data-hf-id="hf-progress-track-${index + 1}" class="progress-track"><div data-hf-id="hf-progress-fill-${index + 1}" id="progress-fill-${index + 1}" class="progress-fill"></div><div data-hf-id="hf-progress-label-${index + 1}" id="progress-label-${index + 1}" class="progress-label${index === 0 ? ' is-current' : ''}" data-layout-allow-occlusion>${index + 1}. ${escapeHtml(chapter.label)}</div></div>
        </div>`).join('');

  const finalSummaryScene = scenes.find((scene) => scene.finalSummary);
  const ctaAt = finalSummaryScene?.ctaAt;
  if (!Number.isFinite(ctaAt)) throw new Error('Missing fixed English CTA timing anchor');

  const timeline = [];
  timeline.push('      tl.set("#opening-cover", { opacity: 1 }, 0);');
  timeline.push('      tl.set("#opening-cover", { opacity: 0 }, 0.8);');
  captions.slice(1).forEach((caption, index) => {
    const at = Math.max(0.18, caption.start);
    const nextAt = captions[index + 2]?.start ?? caption.end;
    const enterDuration = Math.min(0.12, Math.max(0.01, nextAt - at - 0.05));
    timeline.push(`      tl.to("#caption-${index + 1}", { y: -8, opacity: 0, duration: 0.04, ease: "power2.in", overwrite: "auto" }, ${(at - 0.04).toFixed(3)});`);
    timeline.push(`      tl.fromTo("#caption-${index + 2}", { y: 9, opacity: 0 }, { y: 0, opacity: 1, duration: ${enterDuration.toFixed(3)}, ease: "power2.out", immediateRender: false, overwrite: "auto" }, ${at.toFixed(3)});`);
  });
  chapters.forEach((chapter, index) => {
  ...
  timeline.push(`      tl.to("#outro-underline", { scaleX: 1, duration: 0.62, ease: "power3.out" }, ${(ctaAt + 1.884).toFixed(3)});`);
  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing:border-box; }
      ...
      .title { margin:0 0 0 52px; font-size:48px; font-weight:900; line-height:1.24; }
      .story-slot { position:absolute; inset:0; z-index:10; width:1920px; height:1080px; }
      .caption-band { position:absolute; right:132px; bottom:94px; left:132px; z-index:90; height:158px; background:rgba(236,236,234,.985); }
      .caption { position:absolute; inset:0; display:flex; align-items:center; justify-content:center; padding:8px 46px 4px; font-size:46px; font-weight:760; line-height:1.28; text-align:center; opacity:0; }
      ...
      .progress-label { position:absolute; inset:0; z-index:2; display:flex; align-items:center; justify-content:center; overflow:visible; padding:0; color:var(--ink); font-size:20px; font-weight:760; line-height:1; text-align:center; white-space:nowrap; transform:scaleX(.82); transform-origin:center; }
      ...
      .outro-heading { color:var(--blue); font-size:78px; font-weight:900; line-height:1.02; letter-spacing:-2px; }
      .outro-rule { display:block; width:100%; height:5px; margin:44px 0 42px; background:#c9cbc5; }
      .outro-benefit { max-width:790px; margin:0; font-size:68px; font-weight:900; line-height:1.13; letter-spacing:-1.8px; }
      ...
      .opening-cover { position:absolute; inset:0; z-index:400; display:block; width:1920px; height:1080px; object-fit:cover; opacity:1; }
    </style>
  </head>
  <body>
    <div data-hf-id="hf-main-root" id="root" data-composition-id="main" data-start="0" data-duration="${duration.toFixed(3)}" data-width="1920" data-height="1080">
      <img data-hf-id="hf-opening-cover" id="opening-cover" class="opening-cover" src="thumbnails/thumbnail.en-US.png" alt="Why do AI Agents forget long tasks?" data-layout-allow-occlusion>
      ...
      <header data-hf-id="hf-masthead" class="masthead content-layer"><div class="brand">Tiny Agent</div><h1 class="title">Why AI Agents Forget: High-Signal Context for Long Tasks</h1></header>
      <div data-hf-id="hf-caption-band" class="caption-band content-layer" data-layout-allow-overlap>${captionMarkup}
      </div>
      <nav data-hf-id="hf-chapter-progress" class="chapter-progress content-layer" aria-label="Video chapter progress">${progressMarkup}
      </nav>
      <section data-hf-id="hf-outro-overlay" id="outro-overlay" class="outro-overlay" aria-label="Follow Tiny Agent">
        <img class="outro-character" src="assets/images/tiny-agent-outro-key-art-papergray.png" alt="A smiling Tiny Agent waves goodbye">
        <div class="outro-copy">
          <div class="outro-follow-row"><span id="outro-mark" class="outro-mark" aria-hidden="true"></span><h1 class="outro-heading">Follow Tiny Agent</h1></div>
          <span class="outro-rule" aria-hidden="true"></span>
          <p class="outro-benefit"><span>Tiny Agent helps you</span><span>get better at using AI.</span></p>
          <span id="outro-underline" class="outro-underline" aria-hidden="true"></span>
        </div>
      </section>
      <audio data-hf-id="hf-narration" id="narration" class="clip" data-start="0" data-duration="${narrationDuration.toFixed(3)}" data-track-index="10" data-media-start="0" data-volume="1" src="assets/audio/narration.en.normalized.mp3"></audio>
    </div>
    <script>
      window.__timelines = window.__timelines || {};
      const tl = gsap.timeline({ paused: true });
...
async function compile() {
  ...
  normalizeCaptionTimeline(captions);
  ...
  await writeFile(path.join(audioDir, 'narration.en.vtt'), combinedVtt);
  ...
  await writeFile(path.join(projectRoot, 'timing-map.json'), `${JSON.stringify({
    ...
    fixedCta: {
      text: 'Follow Tiny Agent. Tiny Agent helps you get better at using AI.',
  ...
  await writeFile(path.join(projectRoot, 'STORYBOARD.md'), `# Storyboard\n\n| # | Chapter | Time | Layout | Visual focus | Character poses |\n| ---: | --- | --- | --- | --- | --- |\n${storyboardRows}\n`);
  await writeFile(path.join(projectRoot, 'summary.json'), `${JSON.stringify({
    title: 'Why AI Agents Forget: Use High-Signal Context for Long Tasks',
    source: sourcePath,
    language: 'en-US',
    speech: { provider: 'edge-tts', voice, rate, integratedLoudnessTarget: '-20 LUFS mono / approximately -17 LUFS final stereo' },
    alignment: { timingSource: 'edge-tts-vtt-audible-bounds', scriptFirst: true, sceneBoundaries: 'paragraph-to-vtt', propBeats: 'semantic-trigger-v1', openingCoverSeconds: 0.8 },
    composition: { designVersion: 'retention-v2-large-simple', width: 1920, height: 1080, fps: 30, durationSeconds: duration, chapters: chapterTimings, visualStates: scenes.length, captions: captions.length },
    qa: {
      visualBalance: {
    ...
    output: { file: 'renders/2026-07-21-03-agent-context-engineering-retention-v2.en-US.mp4', status: 'pending' },
  }, null, 2)}\n`);
