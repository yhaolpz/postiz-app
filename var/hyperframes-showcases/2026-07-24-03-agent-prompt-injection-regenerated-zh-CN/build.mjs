...
function validateRules(chapters, segments, scenePlan) {
  ...
  for (const chapter of scenePlan.chapters.slice(1, 6)) {
    ...
    if (chapter.scenes.slice(-2).some((scene) => scene.narration.includes('本章小节'))) throw new Error(`${chapter.label} repeats 本章小节 after the first recap point.`);
  }
...
function generateTts() {
  ...
  writeFileSync(path.join(projectDir, 'qa/recap-spoken-prefix-report.json'), `${JSON.stringify({
    pass: scenePlan.chapters.slice(1, 6).every((chapter) => chapter.scenes.at(-3)?.narration.startsWith(profile.audio.chapterRecapSpokenPrefix)
      && chapter.scenes.slice(-2).every((scene) => !scene.narration.includes('本章小节'))),
    requiredPrefix: profile.audio.chapterRecapSpokenPrefix,
    rule: '本章小节 is spoken once before the first point; points two and three begin directly with their number.',
    chapters: scenePlan.chapters.slice(1, 6).map((chapter) => ({
      label: chapter.label,
      firstPoint: { sceneId: chapter.scenes.at(-3)?.id, narration: chapter.scenes.at(-3)?.narration },
      followingPoints: chapter.scenes.slice(-2).map((scene) => ({ sceneId: scene.id, narration: scene.narration }))
    }))
  }, null, 2)}\n`);
