...
const checks = {
  transitions() {
    ...
    if (timing.checkpoints.hookHardCut - timing.checkpoints.firstSentenceEnd > 1 / 30 + 0.0001) failures.push('kinetic hook cut misses first-sentence frame boundary');
  ...
  semantics() {
    ...
    plan.chapters.slice(1, 6).forEach((chapter) => {
      ...
      if (chapter.scenes.slice(-2).some((scene) => scene.narration.includes('本章小节'))) failures.push(`${chapter.label} repeats 本章小节 after the first recap point`);
    });
