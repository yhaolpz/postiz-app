...
function renderPhaseV2(scene, index) {
  if (scene.recap) {
    ...
    const summaryTitle = scene.finalSummary ? 'Use the 4D Card\nKeep judgment' : scene.chapter;
    const summaryCta = '';
    return `
...
<html lang="en-US">
  ...
  <body>
    <div data-hf-id="hf-main-root" id="root" data-composition-id="main" data-start="0" data-duration="${duration.toFixed(3)}" data-width="1920" data-height="1080">
      ...
      <header data-hf-id="hf-masthead" class="masthead content-layer"><div class="brand">Tiny Agent</div><h1 class="title">The 4D AI Fluency Loop</h1></header>
...
async function compile() {
  ...
  await writeFile(path.join(projectRoot, 'timing-map.json'), `${JSON.stringify({
    ...
    fixedCta: {
      ...
      frameErrorSeconds: 0,
      spokenEnd: captions.at(-1)?.end,
      videoEnd: duration,
      tailSeconds: duration - (captions.at(-1)?.end ?? duration),
    },
  ...
  await writeFile(path.join(projectRoot, 'summary.json'), `${JSON.stringify({
    ...
    output: { file: 'renders/2026-07-19-03-ai-fluency-4d-longform.en-US.mp4', status: 'pending' },
  }, null, 2)}\n`);
