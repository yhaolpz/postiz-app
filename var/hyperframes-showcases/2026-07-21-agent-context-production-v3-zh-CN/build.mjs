...
const fps = 30;
const tailPad = 0.3;
const outputName = 'ai-agent-context-production-v3.zh-CN.mp4';
...
function validateRules(chapters, segments, scenePlan) {
  ...
  if (motionScenes.length < 20 || motionTypes.size < 3) throw new Error(`Motion requirement failed: ${motionTypes.size} types / ${motionScenes.length} beats.`);
}
...
function generateTts() {
  ...
  writeFileSync(path.join(projectDir, 'qa/motion-report.json'), `${JSON.stringify({
    pass: new Set(motionScenes.map((scene) => scene.motionType)).size >= 3 && motionScenes.length >= 20,
  ...
  writeFileSync(path.join(projectDir, 'qa/progress-report.json'), `${JSON.stringify({ pass: true, visibleAt: timing.checkpoints.coverTimelineCutAt, coverTimelineCutAt: timing.checkpoints.coverTimelineCutAt, heightPx: 52, widthPx: 1920 }, null, 2)}\n`);
...
function featuredObject(scene, className = '') {
  ...
  return `<div class="featured-object visual-object ${className}"><img src="${propSrc(scene.props[0].id)}" alt=""><div class="object-label">${esc(scene.coreLabel)}</div></div>`;
}
...
function recapMarkup(scene, chapter) {
  ...
  return `<div class="summary-stage"><aside class="summary-side"><span>第 ${scene.chapterNumber} 章小结</span><strong>${esc(scene.chapter)}</strong></aside><div class="summary-body">${recapScenes.map((item, index) => `<div class="summary-point ${index < revealCount ? 'is-visible' : ''} ${index === revealCount - 1 ? 'is-new' : ''}"><b>${index + 1}.</b><span>${esc(item.headline.replace(/^\d+\.\s*/, ''))}</span></div>`).join('')}</div></div>`;
}
...
function renderScene(scene, scenePlan) {
  ...
  if (scene.layout === 'chapter-intro') return `${open}<div class="intro-stage"><div class="intro-copy"><span>第 ${scene.chapterNumber} 章</span><h1>${esc(scene.chapter)}</h1><p>${esc(scene.coreLabel)}</p></div><div class="intro-actors">${actorMarkup(scene, 'human')}${propMarkup(scene.props[0], 'motion-target')}${actorMarkup(scene, 'agent')}</div></div></section>`;
  if (scene.layout === 'generated') return `${open}<h1 class="scene-headline ${headlineClass}">${esc(scene.headline)}</h1><div class="generated-stage"><img class="generated-art visual-object motion-target" src="assets/generated/scene-art/${esc(scene.generatedArt)}" alt="${esc(scene.coreLabel)}"><div class="generated-label">${esc(scene.coreLabel)}</div></div></section>`;
  ...
  if (scene.layout === 'agent-center' || scene.layout === 'human-center') {
    ...
    return `${open}<h1 class="scene-headline ${headlineClass}">${esc(scene.headline)}</h1><div class="solo-stage">${actorMarkup(scene, kind, 'motion-target')}<div class="solo-callout">${esc(scene.coreLabel)}</div></div></section>`;
  }
  if (scene.layout === 'two-props') return `${open}<h1 class="scene-headline ${headlineClass}">${esc(scene.headline)}</h1><div class="pair-stage">${propMarkup(scene.props[0])}<div class="relation-mark">≠</div>${propMarkup(scene.props[1], 'motion-target')}</div><div class="pair-label">${esc(scene.coreLabel)}</div></section>`;
  if (scene.layout === 'big-text') {
    ...
    return `${open}<div class="emphasis-stage"><div class="emphasis-text motion-target">${esc(scene.headline)}</div><div class="emphasis-sub">${esc(scene.coreLabel)}</div>${smallActor}${propMarkup(scene.props[0], 'small-prop')}</div></section>`;
  }
...
function renderHtml(timing, cues, scenePlan) {
  ...
  contentScenes.forEach((scene, index) => {
    ...
    if (scene.type === 'recap') {
      ...
      if (!['agent-center', 'human-center'].includes(scene.layout)) timeline.push(`tl.fromTo("#scene-${scene.id} .visual-object",{scale:.94},{scale:1,duration:.48,stagger:.03,ease:"back.out(1.35)"},${(scene.start + .04).toFixed(3)});`);
    }
    if (scene.motionType) {
      const at = Math.min(scene.end - 1.05, scene.start + Math.max(1, (scene.end - scene.start) * .52));
      if (scene.motionType === 'pulse') timeline.push(`tl.to("#scene-${scene.id} .motion-target",{scale:1.075,duration:.28,yoyo:true,repeat:1,ease:"power2.inOut"},${at.toFixed(3)});`);
      if (scene.motionType === 'bounce') timeline.push(`tl.to("#scene-${scene.id} .motion-target",{y:-28,duration:.27,yoyo:true,repeat:1,ease:"power2.out"},${at.toFixed(3)});`);
      if (scene.motionType === 'slide') {
        timeline.push(`tl.to("#scene-${scene.id} .motion-target",{x:34,duration:.22,ease:"power2.out"},${at.toFixed(3)});`);
        timeline.push(`tl.to("#scene-${scene.id} .motion-target",{x:-16,duration:.2,ease:"power2.inOut"},${(at + .23).toFixed(3)});`);
        timeline.push(`tl.to("#scene-${scene.id} .motion-target",{x:0,duration:.2,ease:"power2.out"},${(at + .44).toFixed(3)});`);
      }
      if (scene.motionType === 'tilt') timeline.push(`tl.to("#scene-${scene.id} .motion-target",{rotation:3.5,duration:.2,yoyo:true,repeat:3,ease:"sine.inOut"},${at.toFixed(3)});`);
    }
  });
...
.intro-stage{height:848px;display:grid;grid-template-columns:600px 1fr;gap:40px}.intro-copy{height:100%;padding:130px 54px 80px;background:var(--blue);color:var(--paper);display:flex;flex-direction:column;justify-content:center}.intro-copy>span{font-size:34px;font-weight:800}.intro-copy h1{margin:24px 0 34px;font-size:90px;line-height:1.03}.intro-copy p{margin:0;font-size:40px;line-height:1.24;font-weight:700}.intro-actors{display:grid;grid-template-columns:390px 330px 390px;align-items:end;justify-content:center}.intro-actors .actor-wrap{width:390px;height:590px}.intro-actors .prop-piece{width:330px;height:330px;align-self:center}
.summary-stage{height:848px;display:grid;grid-template-columns:570px 1fr}.summary-side{background:var(--blue);color:var(--paper);border-right:14px solid #1597EA;padding:120px 54px 80px;display:flex;flex-direction:column;justify-content:center}.summary-side span{font-size:32px;font-weight:800}.summary-side strong{margin-top:28px;font-size:76px;line-height:1.05}.summary-body{padding:74px 64px 48px;display:grid;grid-template-rows:repeat(3,1fr);gap:8px}.summary-point{visibility:hidden;opacity:0;display:grid;grid-template-columns:72px 1fr;align-items:center;border-bottom:3px solid var(--rest);font-size:43px;line-height:1.18}.summary-point.is-visible{visibility:visible;opacity:1}.summary-point b{color:var(--blue);font-size:38px}.summary-point span{font-weight:750}.chip-row{position:absolute;left:50%;bottom:205px;transform:translateX(-50%);display:flex;gap:10px}.chip-row span{padding:8px 14px;border:4px solid var(--ink);border-radius:14px;background:var(--yellow);font-size:32px;font-weight:900}
