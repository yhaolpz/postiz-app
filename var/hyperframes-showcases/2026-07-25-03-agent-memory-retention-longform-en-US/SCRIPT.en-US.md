...
Microsoft Research calls this the raw-history problem: long interaction logs mix useful experience with details that no longer matter.
...
An interaction record can contain a decision, a failed search, copied text, an obsolete plan, and a temporary tool error all at once.
...
That is why larger memory can feel like better recall while producing slower, less reliable reasoning.
...
One useful unit is a fact: a verified constraint, a current setting, or a result that narrows what the Agent can safely do.
...
In practical terms, start with the current goal, the entity, the risk, and the next action that needs support.
...
Distill an item when the event is long but its lesson can be expressed as a fact, rule, checklist, or handoff note.
...
Give every candidate item six fields: the item, its next-task value, its state, its compact form, its retrieval cue, and its review date.

If a deployment constraint changes permissions, retain the verified constraint with the system and action that need it.

If a troubleshooting session reveals a repeatable fix, distill it into a short skill with a trigger, steps, evidence, and stopping condition.
...
The research result is encouraging, but treat it as evidence for testing your own memory design, not as a universal performance guarantee.
