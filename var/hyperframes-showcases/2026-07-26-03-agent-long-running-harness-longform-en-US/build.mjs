...
function buildHookTiming(firstSegment) {
  ...
  const words = tokens.map((token, wordIndex) => {
    ...
    [...token].forEach((char, letterIndex) => {
      const at = wordStart + (nextWordStart - wordStart) * (letterIndex / Math.max(1, token.length - 1));
...
const FALLBACK_HEADLINES = {
  'c01-p05': 'The five-part continuity plan',
  'c02-p06': 'Keep state outside the window',
  'c03-p05': 'Make requirements easy to inspect',
  'c03-p06': 'Prepare the next session before delivery',
  'c04-p05': 'Tie work to a verification signal',
  'c04-p06': 'Split work by observable outcome',
  'c05-p05': 'Record decisions with evidence',
  'c05-p06': 'Link sources instead of copying context',
  'c06-p02': 'Verify outcomes users can experience',
  'c06-p03': 'Evidence before a completion claim',
  'c06-p06': 'Failures become next diagnostics',
  'c07-p01': 'A handoff makes continuity visible',
  'c07-p02': 'Build a verified working floor',
  'c07-p03': 'Choose a bounded next move',
  'c07-p04': 'Write an evidence-backed handoff',
  'c07-p05': 'Verify before claiming progress',
  'c07-p06': 'Keep the next session moving'
};

const COMPACT_HEADLINES = {
  'c01-p01': 'AI Agent across context windows',
  'c01-p02': 'Anthropic tested a long-running setup',
  'c01-p03': 'Restarts burn time and raise risk',
  'c01-p04': 'A small handoff directs the next session',
  'c01-p06': 'Save this progress system for later',
  'c02-p02': 'Too much work or premature completion',
  'c02-p03': 'New sessions need the current state',
  'c02-p04': 'Choose a safe next action',
  'c02-p05': 'Inspect state, uncertainty, and next steps',
  'c02-p08': 'Avoid repeats and premature claims',
  'c02-p09': 'Actionable state beats a transcript',
  'c03-p02': 'Use equivalent anchors in other domains',
  'c03-p03': 'Map the work before driving',
  'c03-p04': 'A feature needs real verification',
  'c03-p06': 'Prepare the next session',
  'c03-p08': 'Expose requirements and constraints',
  'c03-p09': 'Prepare future sessions',
  'c04-p02': 'Fit the next move in context',
  'c04-p03': 'Change, test, then record',
  'c04-p04': 'Small units reduce recovery cost',
  'c05-p02': 'Evidence makes a decision usable',
  'c05-p03': 'Link the feature list and progress',
  'c05-p04': 'A diff is not progress by itself',
  'c05-p06': 'Link sources, not context',
  'c06-p02': 'Verify user-visible outcomes',
  'c06-p04': 'A pass flag is not the proof',
  'c06-p05': 'Turn resets into focused continuations',
  'c07-p07': 'Tiny Agent helps you use AI better'
};
...
function buildScenePlan(chapters, resolvedSegments, duration) {
  ...
  for (const chapter of chapters) {
    const chapterScenes = chapter.paragraphs.map((text, paragraphIndex) => {
      ...
      const headline = COMPACT_HEADLINES[id] || configuredHeadline || FALLBACK_HEADLINES[id] || text;
      const coreLabel = type === 'recap'
        ? (configuredCoreLabel || headline)
        : (COMPACT_HEADLINES[id] || configuredCoreLabel || headline);
...
function hookMarkup(timing) {
  const hookLines = episode.hookLines ?? (episode.locale.startsWith('zh')
...
function renderHtml(timing, cues, scenePlan, narrationDuration) {
  const scenes = scenePlan.chapters.flatMap((chapter) => chapter.scenes);
...
async function compile() {
  ...
  writeFileSync(path.join(projectDir, 'index.html'), `${renderHtml(timing, cues, scenePlan, narrationDuration)}\n`);
