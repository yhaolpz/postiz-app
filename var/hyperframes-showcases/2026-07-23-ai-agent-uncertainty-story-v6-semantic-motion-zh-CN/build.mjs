...
function cueSemanticCategory(text) {
  if (/证据|来源|核对|查到|查询|预报|雷达|事实|SimpleQA|答对|答错|准确率|验证/.test(text)) return 'evidence';
  if (/未知|不能|无法|缺失|缺少|错误|猜|不回答|拒答|风险|停止|过期|不够|没有|不代表|不可答/.test(text)) return 'uncertainty';
  if (/因此|所以|于是|这就|带来|导致|如果|但|而是|因为|才会|仍然/.test(text)) return 'causal';
  if (/采用|行动|建议|批准|撤销|带上|补信息|核验|停下来|允许做|决定/.test(text)) return 'decision';
  if (/两个|两次|不同|一起看|第一个|第二个|另一个|相比|同时包含|分别/.test(text)) return 'comparison';
  if (/伞|天气|下雨|淋|上海|步行|出门|第二天|阵雨/.test(text)) return 'story';
  return 'claim';
}
...
function targetForSemanticNode(scene, category, nodeIndex) {
  const root = `#scene-${scene.id}`;
  const primary = `${root} .motion-target`;
  const secondary = scene.layout === 'generated'
    ? `${root} .generated-label`
    : scene.layout === 'two-props'
      ? `${root} .pair-label`
      : scene.layout === 'big-text'
        ? `${root} .emphasis-sub`
        : scene.layout === 'chapter-intro'
          ? `${root} .intro-copy`
          : ['agent-center', 'human-center'].includes(scene.layout)
            ? `${root} .solo-callout`
            : `${root} .scene-headline`;
  const actor = scene.renderAgent
    ? `${root} .actor-agent`
    : scene.renderHuman
      ? `${root} .actor-human`
      : primary;
  ...
  const selector = category === 'evidence'
    ? primary
    : category === 'uncertainty'
      ...
      : category === 'decision'
        ? actor
        : category === 'comparison'
          ? (nodeIndex % 2 === 0 ? primary : secondary)
          : category === 'claim'
            ? secondary
            : primary;
  return { target: targetNames[category], selector };
}

function semanticPurpose(category, triggerText) {
  const purpose = {
    evidence: '把证据或来源抬到视觉中心，帮助观众识别答案凭什么成立',
    uncertainty: '标记证据缺口或错误风险，提醒观众不要把流畅语气当作事实',
...
function buildSemanticMotionPlan(cues, scenePlan) {
  ...
  const actionOptions = {
    evidence: ['evidence-focus', 'source-connect'],
    uncertainty: ['uncertainty-brake', 'risk-marker'],
    causal: ['causal-link', 'state-transition'],
    decision: ['decision-lock', 'action-gate'],
    comparison: ['outcome-compare', 'contrast-shift'],
    story: ['story-callback', 'object-handoff'],
    claim: ['claim-highlight', 'key-point-lock']
  };
  const nodes = [];
  let previousActions = [];
  for (const scene of eligibleScenes) {
    ...
    selected.forEach((cue, nodeIndex) => {
      ...
      let action = options[(nodeIndex + scene.paragraphNumber) % options.length];
      if (previousActions.length >= 2 && previousActions.at(-1) === action && previousActions.at(-2) === action) {
        action = options.find((candidate) => candidate !== action) || action;
      }
      previousActions.push(action);
      const target = targetForSemanticNode(scene, category, nodeIndex);
      nodes.push({
        ...
        duration: 0.62,
        holdUntil: Number(Math.min(scene.end, cue.start + 1.2).toFixed(3)),
  ...
  for (const node of nodes) {
    currentRun = node.action === previousAction ? currentRun + 1 : 1;
    repeatedActionRun = Math.max(repeatedActionRun, currentRun);
    previousAction = node.action;
  }
  ...
  const requiredFields = ['id', 'sceneId', 'at', 'triggerCueId', 'triggerText', 'target', 'action', 'semanticPurpose', 'duration', 'holdUntil'];
  ...
  const pass = semanticNodesPerMinute >= profile.visual.semanticActionsPerMinuteHard[0]
    ...
    && unboundNodeCount === 0
    && repeatedActionRun <= profile.visual.maximumRepeatedActionRun
  ...
  return {
    ...
    unboundNodeCount,
    maximumRepeatedActionRun: repeatedActionRun,
    sceneMetrics,
    nodes
  };
}
...
function generateTts() {
  ...
  writeFileSync(path.join(projectDir, 'animation-plan.json'), `${JSON.stringify({
    ...semanticMotionPlan,
    scenes: visualScenes.map((scene) => ({
      id: scene.id,
      start: scene.start,
      end: scene.end,
      layout: scene.layout,
      generatedArt: scene.generatedArt,
      entrance: scene.id === 'c01-p02'
        ? 'hook-hard-cut'
        : scene.layout === 'chapter-intro'
          ? 'chapter-vertical-mask'
          : 'semantic-mask-reveal',
      semanticNodes: semanticMotionPlan.nodes.filter((node) => node.sceneId === scene.id).map((node) => node.id)
    }))
  }, null, 2)}\n`);
...
function appendSemanticNodeTimeline(timeline, node) {
  const selector = JSON.stringify(node.selector);
  const at = node.at.toFixed(3);
  const settleAt = (node.at + 0.26).toFixed(3);
  const finalAt = (node.at + 0.52).toFixed(3);
  if (['evidence-focus', 'source-connect'].includes(node.action)) {
    timeline.push(`tl.to(${selector},{scale:1.06,y:-8,duration:.24,ease:"power3.out",overwrite:"auto"},${at});`);
    timeline.push(`tl.to(${selector},{scale:1.015,y:0,duration:.34,ease:"power2.inOut",overwrite:"auto"},${settleAt});`);
  } else if (['uncertainty-brake', 'risk-marker'].includes(node.action)) {
    timeline.push(`tl.to(${selector},{x:-14,rotation:-1.5,scale:1.035,duration:.22,ease:"power2.out",overwrite:"auto"},${at});`);
    timeline.push(`tl.to(${selector},{x:0,rotation:0,scale:1.01,duration:.34,ease:"power3.out",overwrite:"auto"},${settleAt});`);
  } else if (['causal-link', 'state-transition'].includes(node.action)) {
    timeline.push(`tl.to(${selector},{x:18,y:-5,duration:.24,ease:"power2.out",overwrite:"auto"},${at});`);
    timeline.push(`tl.to(${selector},{x:0,y:0,duration:.34,ease:"power3.out",overwrite:"auto"},${settleAt});`);
  } else if (['decision-lock', 'action-gate'].includes(node.action)) {
    timeline.push(`tl.to(${selector},{scale:1.07,duration:.24,ease:"back.out(1.4)",overwrite:"auto"},${at});`);
    timeline.push(`tl.to(${selector},{scale:1.02,duration:.34,ease:"power2.inOut",overwrite:"auto"},${settleAt});`);
  } else if (['outcome-compare', 'contrast-shift'].includes(node.action)) {
    timeline.push(`tl.to(${selector},{rotation:-2.4,duration:.18,ease:"power2.out",overwrite:"auto"},${at});`);
    timeline.push(`tl.to(${selector},{rotation:2.4,duration:.2,ease:"sine.inOut",overwrite:"auto"},${(node.at + 0.19).toFixed(3)});`);
    timeline.push(`tl.to(${selector},{rotation:0,duration:.24,ease:"power2.out",overwrite:"auto"},${(node.at + 0.4).toFixed(3)});`);
  } else if (['story-callback', 'object-handoff'].includes(node.action)) {
    timeline.push(`tl.to(${selector},{y:-14,scale:1.045,duration:.24,ease:"power3.out",overwrite:"auto"},${at});`);
    timeline.push(`tl.to(${selector},{y:0,scale:1.012,duration:.34,ease:"power2.inOut",overwrite:"auto"},${settleAt});`);
  } else {
    timeline.push(`tl.to(${selector},{scale:1.04,duration:.24,ease:"power3.out",overwrite:"auto"},${at});`);
    timeline.push(`tl.to(${selector},{scale:1.012,duration:.34,ease:"power2.inOut",overwrite:"auto"},${settleAt});`);
  }
  timeline.push(`tl.set(${selector},{transformOrigin:"50% 50%"},${finalAt});`);
}
...
function renderHtml(timing, cues, scenePlan, animationPlan) {
  ...
  contentScenes.forEach((scene, index) => {
    if (index === 0) timeline.push(`tl.set("#scene-${scene.id}",{autoAlpha:1,clipPath:"inset(0% 0% 0% 0%)"},0);`);
    else if (index === 1) {
      timeline.push(`tl.set("#scene-${contentScenes[index - 1].id}",{autoAlpha:0},${scene.start.toFixed(3)});`);
      timeline.push(`tl.set("#scene-${scene.id}",{autoAlpha:1,clipPath:"inset(0% 0% 0% 0%)"},${scene.start.toFixed(3)});`);
    } else {
      const previous = contentScenes[index - 1];
      const chapterChange = scene.chapterNumber !== previous.chapterNumber;
      const sameLayout = scene.layout === previous.layout;
      const revealDuration = chapterChange ? 0.75 : sameLayout ? 0.48 : 0.53;
      const initialClip = chapterChange ? 'inset(100% 0% 0% 0%)' : index % 2 === 0 ? 'inset(0% 100% 0% 0%)' : 'inset(0% 0% 0% 100%)';
      timeline.push(`tl.set("#scene-${scene.id}",{autoAlpha:1,clipPath:"${initialClip}"},${Math.max(0, scene.start - 0.02).toFixed(3)});`);
      timeline.push(`tl.to("#scene-${scene.id}",{clipPath:"inset(0% 0% 0% 0%)",duration:${revealDuration},ease:"power3.out"},${scene.start.toFixed(3)});`);
      timeline.push(`tl.set("#scene-${previous.id}",{autoAlpha:0},${(scene.start + revealDuration).toFixed(3)});`);
    }
    if (scene.type === 'recap') {
      ...
      timeline.push(`tl.set("#scene-${scene.id} ${copySelector}",{y:16},${Math.max(0, scene.start - .02).toFixed(3)});`);
      timeline.push(`tl.to("#scene-${scene.id} ${copySelector}",{y:0,duration:.46,ease:"power3.out"},${scene.start.toFixed(3)});`);
      if (scene.renderHuman || scene.renderAgent) {
        timeline.push(`tl.set("#scene-${scene.id} .actor",{y:10},${Math.max(0, scene.start - .02).toFixed(3)});`);
        timeline.push(`tl.to("#scene-${scene.id} .actor",{y:0,duration:.56,stagger:.035,ease:"power2.out"},${(scene.start + .02).toFixed(3)});`);
      }
    }
  });
