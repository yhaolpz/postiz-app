...
async function main() {
  ...
  const metadataArtifact = imported.manifest.artifacts.find(
    (artifact) => artifact.role === 'publishing-metadata'
  );
  assert(metadataArtifact, 'Imported bundle is missing publishing-metadata.');
  const metadataPath = path.join(imported.bundleDir, metadataArtifact.file);
