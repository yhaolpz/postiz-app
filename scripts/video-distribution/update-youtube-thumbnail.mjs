...
async function main() {
  ...
  const thumbnailArtifact = imported.manifest.artifacts.find(
    (artifact) => artifact.role === 'cover-16x9'
  );
  assert(thumbnailArtifact, 'Imported bundle is missing cover-16x9.');
  const thumbnailPath = path.join(imported.bundleDir, thumbnailArtifact.file);
