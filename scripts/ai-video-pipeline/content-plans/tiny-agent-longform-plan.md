...
每天北京时间 `03:00` 消费一个独立选题。每次围绕该选题制作英文和中文两条横屏长视频：英文版通过 Postiz 发布到 YouTube；中文版完成相同级别的内容、音视频和画面准出后，只保留本地成片、通用中文元数据和 `4:3`、`3:4` 两张中文封面，不再生成中文 `16:9` 封面，也不打开或代填任何中文平台上传页。每期同步产出自然中文与自然英文两套本地发布文案和各三条互动建议。
...
| 每次交付         | 每次消费一个独立选题，并交付该主题的英文版和中文版各 `1` 条。中文是内容呈现、场景、画面、动作、节奏和封面格式的主版本；英文共享同一事实、中心结论、P0/P1/P2、章节、例子、边界、最终工具、场景骨架与视觉语义，只替换为自然英文、英文字幕和成人英文旁白。两版分别从各自最终 TTS/VTT 生成绝对秒点，不复用另一语言的措辞或时间戳；自 `2026-07-28-03` 起必须通过 `englishChineseProductionParity` 双语一致性准出 |
...
| 双语封面         | 自 `2026-07-29-03` 起每期固定五张：英文 `thumbnail.en-US.png` 4K `16:9` 发布母版（`3840x2160`）、`thumbnail.en-US.4x3.png`（`1200x900`）、`thumbnail.en-US.3x4.png`（`900x1200`）；中文只生成 `thumbnail.zh-CN.4x3.png`（`1200x900`）和 `thumbnail.zh-CN.3x4.png`（`900x1200`）。英文 `16:9` 另生成 `1280x720` 与 `256x144` QA 预览，`4:3`/`3:4` 分别生成 `240x180`/`180x240` 预览；只向 Postiz 提交英文 4K `16:9` 母版。中英文 `4:3`/`3:4` 使用 active profile 的同一严格几何和主题动作，全部为 sRGB PNG、单张小于 `2 MB`；英文比例扩展与中文两张封面仅本地交付 |
| 双语发布素材     | 每期生成 `local-publishing-materials.zh-CN.json` 和自然英文对应文件 `local-publishing-materials.en-US.json`。两版分别包含标题、简介、hashtag、关键词及恰好三条互动建议（开放问题、实操取舍、观点/经验邀请），并与各自 `publish-metadata` 一致；英文不是机械翻译。两套素材都不得伪造来源、成果或互动数据，也不自动发布；不生成 `xiaohongshuTitle`、平台专用话题、合集或表单状态 |
| 临时图角色身份   | 自 `2026-07-29-03` 起，临时场景图和封面 hero 中出现的所有 Agent 都必须属于固定 `tiny-agent-v2` 角色家族。生成时必须实际传入固定角色 PNG 作为图片参考；允许完全一致，或做与 `2026-07-28` 临时图相当的轻微姿态、表情、配件、线稿/克制软 3D 变化，但圆润白色紧凑轮廓、黑色脸屏、两只蓝眼、单根圆头天线、白黑蓝主配色和友好比例不可改变。高挑人形、重甲机甲、动物、真人脸、其它吉祥物或显著身体/脸部重设计直接阻塞生产 |
...
| 2026-07-27 | Microsoft Research：[From raw interaction to reusable knowledge](https://www.microsoft.com/en-us/research/blog/from-raw-interaction-to-reusable-knowledge-rethinking-memory-for-ai-agents/)，2026-03-10 | Agent 记得越多，为什么反而可能越笨？              | 记忆保留、提炼、遗忘表     | 英文已发布；中文本地成片已生成（RUN_KEY `2026-07-25-03`） |
| 2026-07-28 | Anthropic：[Effective harnesses for long-running agents](https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents)，2025-11-26                                                 | Agent 连续工作几小时，怎样避免跑偏？              | 长任务进度与交接模板       | 英文已发布；中文本地成片已生成（RUN_KEY `2026-07-26-03`） |
| 2026-07-29 | OpenAI：[Using skills](https://openai.com/academy/skills/)，2026-04-10                                                                                                                                  | 怎样把一次成功经验变成下次还能复用的能力？        | Skill 模板                 | 英文已发布；中文本地成片已生成（RUN_KEY `2026-07-27-03`；[YouTube](https://www.youtube.com/watch?v=YjSsSoimeqg)）              |
| 2026-07-30 | Anthropic：[How we built our multi-agent research system](https://www.anthropic.com/engineering/multi-agent-research-system)，2025-06-13                                                                | 一个 Agent 什么时候真的不够用？                   | 单 Agent / 多 Agent 决策树 | 英文已发布；中文本地成片已生成（RUN_KEY `2026-07-28-03`；[YouTube](https://www.youtube.com/watch?v=bppXyaWMNLg)） |
...
→ 分别完成英文版和中文版全部准出检查
→ 查看固定参考图，生成五张固定封面：英文 16:9/4:3/3:4、中文 4:3/3:4
→ 对全部临时场景图和封面 hero 做 Tiny Agent 固定素材身份一致性审查并通过哈希绑定校验
→ 生成中英文两套本地发布文案与各三条互动建议，并通过双语发布素材校验
→ 生成英文发布标题、简介、章节、关键词和话题
→ 仅将英文版通过 Postiz 发布至 YouTube 指定播放列表
→ 验证英文公开视频和播放列表归属
→ 生成通用中文标题、精简简介与 hashtag，不生成小红书专用字段
→ 在任务报告中提供中文版 MP4、两张中文封面，以及中文与自然英文两套发布素材的本地路径与 QA
→ 更新计划状态并写入归档
```
