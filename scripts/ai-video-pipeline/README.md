...
- Use the selected `A - whiteboard stick figure` style: soft warm-white `#F7F7F5` background,
...
- Tiny Agent summaries must record `layoutStandard: cross-platform-balanced-v1`,
  `speech.rate: +20%`, `captions.mode: realtime-vtt`, the fixed CTA, and
  `theme.background: #F7F7F5`. `--reuse-video` rejects Tiny Agent videos that
...
- Tiny Agent canvas, neutral art fills, transition background, and subtitle box
  must use the soft warm-white background `#F7F7F5`; sampled empty corners
...
scene changes use a short main-art-only fade through the soft warm-white whiteboard
...
- Tiny Agent `summary.json` files include the shared `publicCaption`, final
  `youtubeDescription`, and `youtubeTrackingUrl: null`; public copy contains no
  external URL.
...
- `AI_VIDEO_TTS_RATE`: speech rate. Tiny Agent English defaults to `+20%`; for
...
1. YouTube Tiny Agent uploads publish as `public` after the local MP4 QA gates
   pass and are added to the `Tiny Agent` playlist. YouTube, TikTok, and
   Facebook descriptions use concise plain English plus hashtags and contain no
   external URL; sources may be credited as plain text without a link.
