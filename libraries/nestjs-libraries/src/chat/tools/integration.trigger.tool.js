    ...
    run() {
      ...
      Sometimes this tool requires to user prompt for some settings, like a word to search for. methodName is required [input:callable-tools]`,
            ...
            execute: (inputData, context) => __awaiter(this, void 0, void 0, function* () {
                ...
                const getIntegration = yield this._integrationService.getIntegrationById(organizationId, inputData.integrationId);
                if (!getIntegration) {
                    return {
                ...
                if (
                // @ts-ignore
                !tools[integrationProvider.identifier].some((p) => p.methodName === inputData.methodName) ||
                    // @ts-ignore
                ...
                while (true) {
                    try {
                        // @ts-ignore
                        const load = yield integrationProvider[inputData.methodName](getIntegration.token, inputData.dataSchema.reduce((all, current) => (Object.assign(Object.assign({}, all), { [current.key]: current.value })), {}), getIntegration.internalId, getIntegration);
                        return { output: load };
                    }
                    catch (err) {
