...
const { getPostsList, getPost, inAppNotification, changeState, updatePost, sendWebhooks, isCommentable, } = proxyActivities({
    startToCloseTimeout: '10 minute',
    retry: {
        maximumAttempts: 3,
        backoffCoefficient: 1,
...
export function postWorkflowV105(_a) {
    return __awaiter(this, arguments, void 0, function* ({ taskQueue, postId, organizationId, postNow = false, }) {
        ...
        const { postSocial, postComment, getIntegrationById, refreshTokenWithCause, internalPlugs, globalPlugs, processInternalPlug, processPlug, } = proxyTaskQueue(taskQueue);
        let poked = false;
        setHandler(poke, () => {
            poked = true;
        });
        ...
        const firstPost = yield getPost(organizationId, postId);
        // in case doesn't exists for some reason, fail it
        if (!firstPost) {
            yield changeState(postId, 'ERROR', 'No Post');
            return;
        }
        if (!postNow && firstPost.state !== 'QUEUE') {
            yield changeState(firstPost.id, 'ERROR', 'Already posted', [firstPost]);
            return;
        }
        // if it's a repeatable post, we should ignore this.
        if (!postNow) {
            yield sleep(dayjs(firstPost.publishDate).isBefore(dayjs())
                ? 0
        ...
        if ((_b = post.integration) === null || _b === void 0 ? void 0 : _b.refreshNeeded) {
            ...
            yield changeState(postsListBefore[0].id, 'ERROR', 'Refresh channel needed', postsListBefore);
            return;
        }
        ...
        if ((_g = post.integration) === null || _g === void 0 ? void 0 : _g.disabled) {
            ...
            yield changeState(postsListBefore[0].id, 'ERROR', 'Channel disabled', postsListBefore);
            return;
        }
            ...
            for (const _ of iterate) {
                try {
                    ...
                    if (err instanceof ActivityFailure &&
                        ...
                        const refresh = yield refreshTokenWithCause(post.integration, ((_o = err === null || err === void 0 ? void 0 : err.cause) === null || _o === void 0 ? void 0 : _o.message) || '');
                    ...
                    if (err instanceof ActivityFailure &&
                        ...
                        yield inAppNotification(post.organizationId, `Error posting${i === 0 ? ' ' : ' comments '}on ${(_p = post.integration) === null || _p === void 0 ? void 0 : _p.providerIdentifier} for ${(_q = post === null || post === void 0 ? void 0 : post.integration) === null || _q === void 0 ? void 0 : _q.name}`, `An error occurred while posting${i === 0 ? ' ' : ' comments '}on ${(_r = post.integration) === null || _r === void 0 ? void 0 : _r.providerIdentifier}${((_s = err === null || err === void 0 ? void 0 : err.cause) === null || _s === void 0 ? void 0 : _s.message) ? `: ${(_t = err === null || err === void 0 ? void 0 : err.cause) === null || _t === void 0 ? void 0 : _t.message}` : ``}`, true, false, 'fail');
                        return false;
                    }
                }
            }
        ...
        while (list.length > 0) {
            ...
            // wait for the delay
            yield sleep(Math.max(0, Number((_u = todo.delay) !== null && _u !== void 0 ? _u : 0)));
                ...
                for (const _ of iterate) {
                    try {
                        ...
                        if (err instanceof ActivityFailure &&
                            err.cause instanceof ApplicationFailure &&
                            err.cause.type === 'refresh_token') {
                            const refresh = yield refreshTokenWithCause(yield getIntegrationById(organizationId, todo.integration), ((_v = err === null || err === void 0 ? void 0 : err.cause) === null || _v === void 0 ? void 0 : _v.message) || '');
                            if (!refresh || !refresh.accessToken) {
                                break;
                            }
                            continue;
                ...
                for (const _ of iterate) {
                    try {
                        ...
                        if (err instanceof ActivityFailure &&
                            err.cause instanceof ApplicationFailure &&
                            err.cause.type === 'refresh_token') {
                            const refresh = yield refreshTokenWithCause(post.integration, ((_w = err === null || err === void 0 ? void 0 : err.cause) === null || _w === void 0 ? void 0 : _w.message) || '');
                            if (!refresh || !refresh.accessToken) {
                                break;
                            }
                            continue;
            ...
            if (todo.type === 'repeat-post') {
                yield startChild(postWorkflowV105, {
...
//# sourceMappingURL=post.workflow.v1.0.5.js.map