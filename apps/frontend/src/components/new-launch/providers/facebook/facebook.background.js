import { FACEBOOK_PRESETS } from "../../../../../../../libraries/nestjs-libraries/src/dtos/posts/providers-settings/facebook.dto";
...
export function getPresetBackground(id) {
    if (!id)
        return undefined;
    ...
    colors = colors.map((c) => isLight ? mix(c, '#ffffff', 0.5) : isDark ? mix(c, '#000000', 0.32) : c);
    const background = name.includes('gradient') && colors.length >= 2
        ? `linear-gradient(135deg, ${colors.join(', ')})`
        : colors[0];
    const text = luminance(colors[0]) > 0.5 ? '#1c1e21' : '#ffffff';
    return { background, text };
}
//# sourceMappingURL=facebook.background.js.map